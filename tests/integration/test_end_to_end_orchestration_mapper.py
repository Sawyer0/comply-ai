...
def mock_mapper_components():
    ...
    json_validator.parse_output.side_effect = (
        lambda output: MappingResponse.model_validate(json.loads(output))
    )
    ...
    metrics_collector.get_prometheus_metrics.return_value = (
...
class TestEndToEndOrchestrationMapper:
    ...
    def test_mapper_error_handling(self, mapper_app, mock_mapper_components):
        ...
        raw_content = {
            "detector": "unknown",
            "output": "This is a very long piece of raw content that should be rejected because it's too long and looks like raw content rather than a detector output. "
            * 20,
            "tenant_id": "test-tenant",
        }
    ...
    def test_mapper_fallback_behavior(self, mapper_app, mock_mapper_components):
        ...
        mock_mapper_components["json_validator"].validate.return_value = (
            False,
            "Invalid JSON",
        )
