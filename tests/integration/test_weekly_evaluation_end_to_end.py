...
from src.llama_mapper.analysis.domain.services import (
    QualityService,
    WeeklyEvaluationService,
)
from src.llama_mapper.analysis.infrastructure.report_generator import (
...
class TestWeeklyEvaluationEndToEnd:
    ...
    async def test_complete_workflow_from_cli_to_notification(
        ...
        with patch.object(
            weekly_eval_service, "_get_recent_analysis_data"
        ) as mock_get_data:
            mock_get_data.return_value = []  # Empty data for simplicity

            result = cli_command.run(schedule_id=schedule_id["schedule_id"], force=True)
        ...
        assert alerting_system.send_evaluation_notification.call_count == len(
            recipients
        )
    ...
    async def test_cli_list_and_status_commands(
        ...
        schedule1 = cli_command.schedule(
            tenant_id="tenant-1", cron_schedule="0 9 * * 1"
        )
        schedule2 = cli_command.schedule(
            tenant_id="tenant-2", cron_schedule="0 10 * * 2"
        )
    ...
    @pytest.mark.asyncio
    async def test_cli_cancel_command(self, complete_system_setup, mock_config_manager):
        ...
        schedule = cli_command.schedule(
            tenant_id="tenant-cancel", cron_schedule="0 9 * * 1"
        )
        ...
        # Cancel schedule
        success = cli_command.cancel(schedule_id=schedule["schedule_id"], confirm=True)
    ...
    @pytest.mark.asyncio
    async def test_configuration_loading_and_validation(self, temp_storage_dir):
    ...
    async def test_error_handling_and_recovery(
        ...
        with pytest.raises(Exception, match="Invalid cron schedule"):
            cli_command.schedule(tenant_id="test-tenant", cron_schedule="invalid-cron")

        # Test invalid email addresses
        with pytest.raises(Exception, match="Invalid email address"):
            cli_command.schedule(tenant_id="test-tenant", recipients=["invalid-email"])
    ...
    @pytest.mark.asyncio
    async def test_report_generation_formats(self, complete_system_setup):
        ...
        json_report = report_generator.generate_report(
            report_data=report_data, format_type="JSON", tenant_id="test-tenant"
        )
        ...
        csv_report = report_generator.generate_report(
            report_data=report_data, format_type="CSV", tenant_id="test-tenant"
        )
        ...
        pdf_report = report_generator.generate_report(
            report_data=report_data, format_type="PDF", tenant_id="test-tenant"
        )
    ...
    @pytest.mark.asyncio
    async def test_storage_persistence_across_restarts(self, temp_storage_dir):
        ...
        schedule_id = await service1.schedule_weekly_evaluation(
            tenant_id="persistence-test", cron_schedule="0 9 * * 1"
        )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_evaluation_handling(self, complete_system_setup):
        ...
        for i in range(3):
            schedule_id = await weekly_eval_service.schedule_weekly_evaluation(
                tenant_id=f"concurrent-tenant-{i}", cron_schedule="0 9 * * 1"
            )
