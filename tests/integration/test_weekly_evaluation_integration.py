...
class TestWeeklyEvaluationIntegration:
    ...
    async def test_evaluation_with_quality_issues(
        ...
        schedule_id = await service_with_issues.schedule_weekly_evaluation(
            tenant_id=tenant_id, report_recipients=["admin@example.com"]
        )

        with patch.object(
            service_with_issues, "_get_recent_analysis_data"
        ) as mock_get_data:
    ...
    async def test_evaluation_failure_handling(
        ...
        weekly_eval_service.quality_service.evaluate_quality.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_schedule_management(self, weekly_eval_service, storage_backend):
        ...
        schedule1 = await weekly_eval_service.schedule_weekly_evaluation(
            tenant_id=tenant_id, cron_schedule="0 9 * * 1"
        )
        schedule2 = await weekly_eval_service.schedule_weekly_evaluation(
            tenant_id=tenant_id, cron_schedule="0 10 * * 2"
        )
        ...
        tenant_schedules = await weekly_eval_service.list_scheduled_evaluations(
            tenant_id
        )
        ...
        remaining_schedules = await weekly_eval_service.list_scheduled_evaluations(
            tenant_id
        )
    ...
    @pytest.mark.asyncio
    async def test_report_generation_formats(self, report_generator):
    ...
    @pytest.mark.asyncio
    async def test_storage_backend_persistence(self, storage_backend):
    ...
    @pytest.mark.asyncio
    async def test_validation_and_error_handling(self, weekly_eval_service):
        ...
        with pytest.raises(ValueError, match="Invalid cron schedule"):
            await weekly_eval_service.schedule_weekly_evaluation(
                tenant_id="test-tenant", cron_schedule="invalid-cron"
            )

        # Test invalid email addresses
        with pytest.raises(ValueError, match="Invalid email address"):
            await weekly_eval_service.schedule_weekly_evaluation(
                tenant_id="test-tenant", report_recipients=["invalid-email"]
            )
        ...
        success = await weekly_eval_service.cancel_scheduled_evaluation(
            "nonexistent-id"
        )
