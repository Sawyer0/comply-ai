...
class TestRunbookExecutor:
    ...
    def executor(self):
        ...
        with patch('runbook_executor.AnalysisMetricsCollector'), \
    ...
    async def test_check_schema_validation_rate_error(self, executor):
        ...
        # Mock error
        executor.metrics_collector.get_metric.side_effect = Exception("Metric not found")
    ...
    async def test_get_recent_validation_failures(self, executor):
        ...
        executor.validator.get_recent_validation_failures.return_value = [mock_failure1, mock_failure2]
    ...
    async def test_get_opa_compilation_errors(self, executor):
        ...
        executor.opa_generator.get_recent_compilation_errors.return_value = [mock_error1, mock_error2]
    ...
    async def test_execute_schema_validation_investigation(self, executor):
        ...
        executor.check_schema_validation_rate = AsyncMock(return_value={"rate": 0.85, "status": "critical"})
        executor.get_recent_validation_failures = AsyncMock(return_value=[
    ...
    async def test_execute_template_fallback_investigation(self, executor):
        ...
        # Mock investigation methods
        executor.check_template_fallback_rate = AsyncMock(return_value={"rate": 0.35, "status": "critical"})
        executor.check_model_server_health = AsyncMock(return_value={"model_loaded": False, "error_rate": 0.15})
    ...
    async def test_execute_opa_compilation_investigation(self, executor):
        ...
        # Mock investigation methods
        executor.check_opa_compilation_rate = AsyncMock(return_value={"rate": 0.85, "status": "critical"})
        executor.get_opa_compilation_errors = AsyncMock(return_value=[
    ...
    async def test_execute_cost_anomaly_investigation(self, executor):
        ...
        # Mock investigation methods
        executor.check_cost_metrics = AsyncMock(return_value={"daily_cost": 1200.0, "usage_percentage": 120.0, "status": "critical"})
        executor.get_cost_breakdown = AsyncMock(return_value={"cpu": 800.0, "memory": 400.0})
        executor.get_cost_anomalies = AsyncMock(return_value=[
            {"description": "CPU spike detected", "severity": "high", "cost_impact": 200.0}
        ])
    ...
    def test_generate_incident_report_cost_anomaly(self, executor):
        ...
        results = {
            ...
            "anomalies": [
                {"description": "CPU spike detected", "severity": "high"}
            ]
        }
