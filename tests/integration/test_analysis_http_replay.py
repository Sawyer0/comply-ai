...
class HTTPReplayTester:
    ...
    def run_replay_test(self, test_case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Validate response
            result = self._validate_response(test_id, response, expected_response)
    ...
    def _validate_response(
        self, test_id: str, actual_response, expected_response: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _validate_response_body(
        self, actual_body: Dict[str, Any], expected_body: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for field_name, expected_value in expected_body.items():
            ...
            if field_name == "timestamp":
                ...
                if isinstance(actual_value, dict):
                    ...
                    missing_fields = [
                        f for f in required_fields if f not in actual_value
                    ]
...
class TestAnalysisHTTPReplay:
    ...
    def analysis_app(self):
        ...
        with patch(
            "src.llama_mapper.analysis.infrastructure.model_server.Phi3AnalysisModelServer"
        ) as mock_model_server:
            with patch(
                "src.llama_mapper.analysis.infrastructure.validator.AnalysisValidator"
            ) as mock_validator:
                with patch(
                    "src.llama_mapper.analysis.infrastructure.templates.AnalysisTemplateProvider"
                ) as mock_templates:
                    with patch(
                        "src.llama_mapper.analysis.infrastructure.security.AnalysisSecurityValidator"
                    ) as mock_security:

                        # Configure mocks
                        mock_model_server.return_value.analyze = AsyncMock(
                            return_value={
                        ...
                        mock_validator.return_value.validate_and_fallback = Mock(
                            return_value={
                                "reason": "coverage gap detected",
                                "remediation": "add secondary detector",
                                "confidence": 0.85,
                        ...
                        mock_templates.return_value.get_template = Mock(
                            return_value={
                        ...
                        mock_security.return_value.validate_response_security = Mock(
    ...
    def test_health_check_live(self, replay_tester):
        ...
        test_case = {
            ...
            "method": "GET",
            "request": {"headers": {"X-API-Key": "test-api-key-123"}, "body": None},
            "expected_response": {
                "status_code": 200,
                "headers": {"Content-Type": "application/json"},
                "body": {
    ...
    def test_all_golden_cases(self, replay_tester):
        ...
        ), f"Pass rate too low: {summary['pass_rate']:.2%}"

        # Check that we ran the expected number of tests
        assert (
            summary["total_tests"] >= 5
        ), f"Expected at least 5 tests, got {summary['total_tests']}"
