...
class TestOrchestrationEndpoints:
    ...
    def test_orchestrate_endpoint_success(
        self, mock_factory, test_client, sample_orchestration_request
    ):
        """Test successful orchestration endpoint."""
        # Mock the factory components
        mock_factory.router.route_request = AsyncMock(
            return_value=(
                RoutingPlan(
                ...
                RoutingDecision(
                    ...
                    routing_reason="test",
                    health_status={
                        "mock-detector-1": "healthy",
                        "mock-detector-2": "healthy",
                    },
                ),
            )
        )

        mock_factory.run_pipeline = AsyncMock(
    ...
    def test_orchestrate_endpoint_validation_error(self, test_client):
        ...
        response = test_client.post(
            "/orchestrate", json=invalid_request, headers={"X-API-Key": "test-key"}
        )
    ...
    def test_orchestrate_endpoint_missing_tenant(self, test_client):
        ...
        response = test_client.post(
            "/orchestrate", json=request_data, headers={"X-API-Key": "test-key"}
        )
...
class TestJobManagement:
    ...
    def test_job_status_check(self, mock_factory, test_client):
        ...
        response = test_client.get(
            "/orchestrate/status/job-123", headers={"X-API-Key": "test-key"}
        )
...
class TestDetectorManagement:
    ...
    def test_list_detectors(self, mock_factory, test_client):
        ...
        mock_factory.settings.detectors = {
            "detector-1": Mock(
                name="detector-1",
                model_dump=Mock(
                    return_value={"name": "detector-1", "endpoint": "http://detector-1"}
                ),
            ),
            "detector-2": Mock(
                name="detector-2",
                model_dump=Mock(
                    return_value={"name": "detector-2", "endpoint": "http://detector-2"}
                ),
            ),
            "detector-3": Mock(
                name="detector-3",
                model_dump=Mock(
                    return_value={"name": "detector-3", "endpoint": "http://detector-3"}
                ),
            ),
        }

        response = test_client.get("/detectors", headers={"X-API-Key": "test-key"})
    ...
    def test_register_detector(self, mock_factory, test_client):
        ...
        response = test_client.post(
            "/detectors", json=detector_data, headers={"X-API-Key": "test-key"}
        )
...
class TestPolicyManagement:
    ...
    def test_list_policies(self, mock_factory, test_client):
        ...
        response = test_client.get(
            "/policies/test-tenant", headers={"X-API-Key": "test-key"}
        )
    ...
    def test_submit_policy(self, mock_factory, test_client):
        ...
        policy_data = {
            "policy": {
                "rules": [
                    {"detector": "detector-1", "threshold": 0.8, "action": "flag"}
                ]
            },
