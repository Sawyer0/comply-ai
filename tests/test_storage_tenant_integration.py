...
from llama_mapper.storage.tenant_isolation import TenantIsolationManager, TenantContext, TenantAccessLevel
...
class TestStorageTenantIntegration:
    ...
    async def test_retrieve_record_with_tenant_filtering(self, settings, sample_record):
        ...
        scoped_id = storage_manager.tenant_manager.create_tenant_scoped_record_id("tenant-1", sample_record.id)
        
        # Retrieve record using tenant-scoped ID
        retrieved_record = await storage_manager.retrieve_record(scoped_id, tenant_context)
    ...
    def test_tenant_scoped_record_ids(self, settings):
        ...
        scoped_id = storage_manager.tenant_manager.create_tenant_scoped_record_id("tenant-1", base_id)
        
        assert scoped_id.startswith("tenant-1:")
        assert base_id in scoped_id
        
        # Test extraction
        extracted_tenant, extracted_id = storage_manager.tenant_manager.extract_tenant_from_record_id(scoped_id)
