...
from src.llama_mapper.analysis import AnalysisRequest, AnalysisResponse, VersionInfo
...
class TestAnalysisRequestValidation:
    ...
    def test_field_length_constraints(self):
        ...
        assert "at most 64 characters" in str(
            exc_info.value
        ) or "String should have at most 64 characters" in str(exc_info.value)
    ...
    def test_required_detectors_constraints(self):
        ...
        assert "at least 1 items" in str(
            exc_info.value
        ) or "List should have at least 1 item" in str(exc_info.value)

        # Test max_items
        request_data["required_detectors"] = [
            f"detector{i}" for i in range(21)
        ]  # 21 items violates max_items=20

        with pytest.raises(ValidationError) as exc_info:
            AnalysisRequest(**request_data)

        assert "at most 20 items" in str(
            exc_info.value
        ) or "List should have at most 20 items" in str(exc_info.value)
...
class TestAnalysisResponseValidation:
    ...
    def test_valid_analysis_response(self):
        ...
        version_info = VersionInfo(
            taxonomy="v1.0", frameworks="SOC2-v2.0", analyst_model="phi3-mini-v1.0"
        )
    ...
    def test_response_field_constraints(self):
        ...
        assert "less than or equal to 1.0" in str(
            exc_info.value
        ) or "Input should be less than or equal to 1" in str(exc_info.value)
