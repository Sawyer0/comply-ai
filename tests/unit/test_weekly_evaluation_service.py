...
class TestWeeklyEvaluationService:
    ...
    async def test_run_scheduled_evaluation_failure(
        ...
        with patch.object(
            ...
            mock_storage_backend.store_evaluation_result.assert_called_once()
            failure_result = mock_storage_backend.store_evaluation_result.call_args[0][
                0
            ]
    ...
    @pytest.mark.asyncio
    async def test_list_scheduled_evaluations(self, weekly_eval_service):
        ...
        tenant1_schedules = await weekly_eval_service.list_scheduled_evaluations(
            "tenant1"
        )
    ...
    @pytest.mark.asyncio
    async def test_cancel_nonexistent_schedule(self, weekly_eval_service):
    ...
    async def test_send_evaluation_notifications(
        ...
        assert mock_alerting_system.send_evaluation_notification.call_count == len(
            recipients
        )
