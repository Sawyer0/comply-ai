...
class TestAPIKeyManager:
    ...
    def test_validate_api_key(self):
        ...
        # Create a key
        request = APIKeyRequest(
            tenant_id="test-tenant", name="test-key", scopes=["analyze"]
        )
    ...
    def test_validate_invalid_api_key(self):
        ...
        validated_key = manager.validate_api_key("invalid-key", [APIKeyScope.ANALYZE])
    ...
    def test_validate_insufficient_scopes(self):
        ...
        request = APIKeyRequest(
            tenant_id="test-tenant", name="test-key", scopes=["read"]  # Only read scope
        )
        ...
        validated_key = manager.validate_api_key(response.api_key, [APIKeyScope.ADMIN])
    ...
    def test_list_tenant_keys(self):
        ...
        for i in range(3):
            request = APIKeyRequest(
                tenant_id="test-tenant", name=f"test-key-{i}", scopes=["analyze"]
            )
...
class TestAPIKeyAuthDependency:
    ...
    def auth_dependency(self, mock_api_key_manager):
        ...
        return APIKeyAuthDependency(
            api_key_manager=mock_api_key_manager, required_scopes={APIKeyScope.ANALYZE}
        )

    @pytest.mark.asyncio
    async def test_successful_authentication(
        self, auth_dependency, mock_api_key_manager
    ):
        ...
        mock_api_key_manager.validate_api_key.assert_called_once_with(
            "test-api-key", [APIKeyScope.ANALYZE]
        )
    ...
    @pytest.mark.asyncio
    async def test_authentication_failure_invalid_key(
        self, auth_dependency, mock_api_key_manager
    ):
