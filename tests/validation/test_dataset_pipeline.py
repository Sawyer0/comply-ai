...
def main():
    ...
    try:
        ...
        training_examples = training_generator.generate_training_examples(
            examples_per_mapping=2, include_multi_label=True, include_variations=True
        )
        ...
        synthetic_generator = SyntheticDataGenerator(
            taxonomy_loader=taxonomy_loader, random_seed=42
        )

        synthetic_generator.load_taxonomy()

        # Generate different types of synthetic data
        pii_examples = synthetic_generator.generate_synthetic_pii_examples(
            num_examples=30
        )
        jailbreak_examples = synthetic_generator.generate_synthetic_jailbreak_examples(
            num_examples=15
        )
        injection_examples = (
            synthetic_generator.generate_synthetic_prompt_injection_examples(
                num_examples=20
            )
        )
        ...
        print(
            f"   ✅ Validation completed with overall score: {validation_report['overall_score']:.2f}/100"
        )
        ...
        synthetic_stats = synthetic_generator.get_synthetic_statistics(
            balanced_examples
        )
        ...
        print(
            f"  - Format validation: {format_val['valid_examples']}/{format_val['valid_examples'] + format_val['invalid_examples']} valid"
        )
        print(
            f"  - Taxonomy validation: {taxonomy_val['valid_labels']} valid labels, {taxonomy_val['invalid_labels']} invalid"
        )
        ...
        validator.export_validation_report(
            validation_report, "validation_report.json", format="json"
        )
        print("   ✅ Saved validation report to validation_report.json")

        validator.export_validation_report(
            validation_report, "validation_report.txt", format="txt"
        )
        ...
        if all_examples:
            ...
            if single_validation["valid"]:
                ...
                print(
                    f"   ❌ Sample example validation failed: {single_validation['errors']}"
                )
        ...
        print(
            f"  📈 Overall validation score: {validation_report['overall_score']:.2f}/100"
        )
        print(
            f"  🏷️  Taxonomy labels covered: {coverage['covered_taxonomy_labels']}/{coverage['total_taxonomy_labels']}"
        )
        print(
            f"  🔍 Detectors covered: {coverage['covered_detectors']}/{coverage['total_detectors']}"
        )
