...
class QualityMetric:
    ...
    def __post_init__(self):
        ...
        if self.timestamp > now + timedelta(seconds=1):
            raise ValueError("Metric timestamp cannot be more than 1 second in the future")
...
class QualityThreshold:
    ...
    def __post_init__(self):
        ...
        if self.warning_threshold <= 0:
            raise ValueError(f"Warning threshold must be positive: {self.warning_threshold}")
        
        if self.critical_threshold <= 0:
            raise ValueError(f"Critical threshold must be positive: {self.critical_threshold}")
        ...
        if self.time_window_minutes < 1:
            raise ValueError(f"Time window must be at least 1 minute: {self.time_window_minutes}")
...
class Alert:
    ...
    def __post_init__(self):
        ...
        if self.acknowledged_at and self.acknowledged_at < self.created_at:
            raise ValueError("Acknowledged timestamp cannot be before created timestamp")
