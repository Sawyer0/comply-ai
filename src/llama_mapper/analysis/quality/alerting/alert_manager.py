...
from ..interfaces import (
    IAlertManager, Alert, AlertSeverity, AlertStatus,
    QualityMetricType, DegradationDetection
)
...
class AlertManager(IAlertManager):
    ...
    def __init__(
        ...
        logger.info(f"Alert manager initialized with {max_alerts} max alerts")
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_alerts = [a for a in self._alerts.values() if a.created_at >= recent_cutoff]
    ...
    def _should_deduplicate_alert(self, alert: Alert) -> bool:
        ...
        cutoff_time = datetime.now() - timedelta(minutes=self.deduplication_window_minutes)
