...
from ..interfaces import (
    IQualityDetector, QualityMetric, QualityMetricType,
    QualityThreshold, DegradationDetection, DegradationType, AlertSeverity
)
...
class QualityDegradationDetector(IQualityDetector):
    ...
    def __init__(
        ...
        logger.info(f"Quality degradation detector initialized with sensitivity {anomaly_sensitivity}")
    ...
    def detect_degradation(
        ...
        threshold_detection = self._detect_threshold_breach(metric_type, metrics, threshold)
    ...
    def detect_anomalies(
        ...
        for i, metric in enumerate(metrics):
            ...
            if z_score > self.anomaly_sensitivity:
                ...
                detection = DegradationDetection(
                    ...
                    expected_value=mean,
                    deviation_percentage=((metric.value - mean) / mean) * 100 if mean != 0 else 0,
        ...
        logger.debug(f"Detected {len(anomalies)} anomalies in {metric_type.value}")
        return anomalies
    ...
    def detect_trends(
        ...
        if r_squared > 0.7:  # Strong correlation
            trend_type = DegradationType.GRADUAL_DECLINE if slope < 0 else DegradationType.TREND_REVERSAL
            ...
            detection = DegradationDetection(
                ...
                deviation_percentage=((current_value - expected_value) / expected_value) * 100 if expected_value != 0 else 0,
        ...
        logger.debug(f"Detected {len(trends)} trends in {metric_type.value}")
        return trends
    ...
    def _detect_threshold_breach(
        ...
        critical_breaches = [m for m in recent_metrics if m.value <= threshold.critical_threshold]
        if critical_breaches:
            ...
            return DegradationDetection(
                ...
                deviation_percentage=((latest_breach.value - threshold.critical_threshold) / threshold.critical_threshold) * 100,
                confidence=1.0,
        ...
        warning_breaches = [m for m in recent_metrics if m.value <= threshold.warning_threshold]
        if warning_breaches:
            ...
            return DegradationDetection(
                ...
                deviation_percentage=((latest_breach.value - threshold.warning_threshold) / threshold.warning_threshold) * 100,
                confidence=0.8,
                timestamp=latest_breach.timestamp,
    ...
    def _detect_sudden_drop(
        ...
        for i in range(2, len(sorted_metrics)):
            ...
            drop_percentage = ((previous.value - current.value) / previous.value) * 100 if previous.value != 0 else 0
            ...
            if drop_percentage > 20:
                ...
                return DegradationDetection(
                    ...
                    metadata={
                        ...
                        "time_window": (current.timestamp - previous.timestamp).total_seconds()
                    }
                )
    ...
    def _detect_gradual_decline(
        ...
        if slope < -0.01 and r_squared > 0.5:  # Negative slope with good correlation
            ...
            decline_percentage = (total_decline / values[0]) * 100 if values[0] != 0 else 0
