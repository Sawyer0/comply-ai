...
from ..interfaces import (
    IQualityMonitor, QualityMetric, QualityMetricType
)
...
class QualityMonitor(IQualityMonitor):
    ...
    def __init__(
        ...
        if default_retention_hours <= 0:
            raise ValueError(f"default_retention_hours must be positive: {default_retention_hours}")
        
        if cleanup_interval_minutes <= 0:
            raise ValueError(f"cleanup_interval_minutes must be positive: {cleanup_interval_minutes}")
    ...
    def record_metric(self, metric: QualityMetric) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to record metric {metric.metric_type.value}: {e}")
    ...
    def get_metric_trends(
        ...
        with self._lock:
            ...
            volatility = (sum((c - sum(changes)/len(changes))**2 for c in changes) / len(changes))**0.5 if changes else 0.0
