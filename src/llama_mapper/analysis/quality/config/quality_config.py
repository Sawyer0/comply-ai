...
from ..interfaces import AlertSeverity, QualityMetricType, QualityThreshold
...
class QualityAlertingConfig(BaseModel):
    ...
    monitoring_interval_seconds: int = Field(
        60, description="Monitoring interval in seconds"
    )
    ...
    deduplication_window_minutes: int = Field(
        15, description="Alert deduplication window in minutes"
    )

    # Alert handlers
    email_config: Optional[EmailConfig] = Field(
        None, description="Email alert configuration"
    )
    slack_config: Optional[SlackConfig] = Field(
        None, description="Slack alert configuration"
    )
    webhook_config: Optional[WebhookConfig] = Field(
        None, description="Webhook alert configuration"
    )

    # Quality thresholds
    thresholds: List[QualityThresholdConfig] = Field(
        default_factory=list, description="Quality thresholds"
    )

    # Detection settings
    anomaly_sensitivity: float = Field(2.0, description="Anomaly detection sensitivity")
    trend_window_minutes: int = Field(
        30, description="Trend analysis window in minutes"
    )
    min_samples_for_detection: int = Field(
        5, description="Minimum samples for detection"
    )
