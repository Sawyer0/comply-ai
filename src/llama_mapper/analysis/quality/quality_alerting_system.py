...
from .interfaces import (
    IQualityAlertingSystem, QualityMetric, QualityMetricType,
    QualityThreshold, AlertSeverity, DegradationDetection
)
...
class QualityAlertingSystem(IQualityAlertingSystem):
    ...
    def __init__(
        ...
        if monitoring_interval_seconds <= 0:
            raise ValueError(f"monitoring_interval_seconds must be positive: {monitoring_interval_seconds}")
        if max_metrics_per_type <= 0:
            raise ValueError(f"max_metrics_per_type must be positive: {max_metrics_per_type}")
        if alert_retention_days <= 0:
            raise ValueError(f"alert_retention_days must be positive: {alert_retention_days}")
        if deduplication_window_minutes <= 0:
            raise ValueError(f"deduplication_window_minutes must be positive: {deduplication_window_minutes}")
    ...
    def remove_threshold(self, metric_type: QualityMetricType) -> None:
        ...
        if metric_type in self.thresholds:
            ...
            logger.info(f"Removed threshold for {metric_type.value}")
    ...
    def process_metric(self, metric: QualityMetric) -> None:
        ...
        try:
            ...
            logger.debug(f"Processed metric: {metric.metric_type.value}={metric.value}")
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "uptime_seconds": (datetime.now() - self._stats["system_start_time"]).total_seconds()
        }
    ...
    async def send_evaluation_notification(
        ...
        try:
            ...
            evaluation_alert = Alert(
                alert_id=f"eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                title=alert_title,
                message=alert_message,
                severity=severity,
                metric_type=QualityMetricType.CUSTOM_METRIC,
                metric_value=quality_metrics.rubric_score if hasattr(quality_metrics, 'rubric_score') else 0.0,
                ...
                metadata={
                    ...
                    "recipient": recipient,
                    "alerts": alerts,
                    "quality_metrics": quality_metrics.dict() if hasattr(quality_metrics, 'dict') else {},
                }
            )
                ...
                if handler.can_handle_alert(evaluation_alert):
                    ...
                    try:
                        ...
                        if hasattr(handler, 'send_to_recipient'):
                            success = await handler.send_to_recipient(recipient, evaluation_alert)
                        else:
                            success = await handler.send_alert(evaluation_alert)
                        
                        if success:
                            success_count += 1
                            logger.info(f"Sent evaluation notification to {recipient} via {handler.get_handler_name()}")
                        else:
                            logger.warning(f"Failed to send evaluation notification to {recipient} via {handler.get_handler_name()}")
                    
                    except Exception as e:
                        logger.error(f"Error sending evaluation notification via {handler.get_handler_name()}: {e}")
            
            # Store alert in manager
            self.alert_manager.create_alert(evaluation_alert)
            
            success_rate = success_count / total_handlers if total_handlers > 0 else 0
            logger.info(f"Evaluation notification sent to {recipient}: {success_count}/{total_handlers} handlers succeeded")
            
            return success_rate > 0
            
        except Exception as e:
            logger.error(f"Failed to send evaluation notification to {recipient}: {e}")
            return False
    ...
    def _format_evaluation_message(
        ...
        if hasattr(quality_metrics, 'schema_valid_rate'):
            message_parts.append(f"• Schema Validation Rate: {quality_metrics.schema_valid_rate:.2%}")
        ...
        if hasattr(quality_metrics, 'opa_compile_success_rate'):
            message_parts.append(f"• OPA Compilation Success Rate: {quality_metrics.opa_compile_success_rate:.2%}")
        if hasattr(quality_metrics, 'evidence_accuracy'):
            message_parts.append(f"• Evidence Accuracy: {quality_metrics.evidence_accuracy:.2f}")
    ...
    def _perform_monitoring_check(self) -> None:
        ...
        for metric_type in self.quality_monitor._current_metrics.keys():
            try:
                ...
                logger.error(f"Error checking trends for {metric_type.value}: {e}")
        
        # Cleanup old alerts periodically
        if self._stats["last_monitoring_check"] is None or \
           (datetime.now() - self._stats["last_monitoring_check"]).total_seconds() > 3600:  # 1 hour
        ...
        try:
            ...
            if self.alert_handlers:
                ...
                if success:
                    ...
                    logger.warning(f"Quality degradation alert sent: {title}")
                else:
                    logger.error(f"Failed to send quality degradation alert: {title}")
            else:
                # No handlers configured, just log
                logger.warning(f"Quality degradation detected (no handlers): {title}")
            
        except Exception as e:
            logger.error(f"Error handling degradation detection: {e}")
