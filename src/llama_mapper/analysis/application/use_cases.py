...
class AnalyzeMetricsUseCase:
    ...
    async def execute(self, request_dto: AnalysisRequestDTO) -> AnalysisResponseDTO:
        ...
        try:
            ...
            # Validate request
            if not self.validation_service.validate_analysis_request(request.dict()):
                raise ValueError("Request validation failed")

            # Process analysis
            response = await self.analysis_service.analyze(request)

            # Convert domain entity to DTO
            return AnalysisResponseDTO.from_domain_entity(response)

        except ValueError as e:
            logger.error("Validation error in analyze metrics use case: %s", e)
            raise
        except Exception as e:
            logger.error("Analysis error in analyze metrics use case: %s", e)
    ...
    async def execute(
        ...
        try:
            ...
            for request in batch_request.requests:
                if not self.validation_service.validate_analysis_request(
                    request.dict()
                ):
                    raise ValueError(
                        f"Request validation failed for request {request.request_id}"
                    )
            ...
            logger.error("Validation error in batch analyze metrics use case: %s", e)
            raise
        except Exception as e:
            logger.error(
                "Batch analysis error in batch analyze metrics use case: %s", e
            )
...
class HealthCheckUseCase:
    ...
    def _check_opa_generator(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_policy = self.opa_generator.generate_coverage_policy(
                ["test"], {"test": 0.8}
            )
            is_valid = self.opa_generator.validate_rego(test_policy)
            return {
                "status": "healthy" if is_valid else "unhealthy",
                "message": "OPA generator operational",
            }
        except Exception as e:
            return {"status": "unhealthy", "message": str(e)}
