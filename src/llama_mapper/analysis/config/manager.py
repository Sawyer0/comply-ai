...
from .settings import AnalysisConfig, AnalysisSettings
...
class AnalysisConfigManager(ConfigManager):
    ...
    def _load_analysis_config(self):
        ...
        try:
            ...
            logger.error("Failed to load analysis configuration: %s", e)
            raise
    ...
    def validate_config(self) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            if not os.path.exists(settings.analysis_model_path):
                errors.append(
                    f"Model path does not exist: {settings.analysis_model_path}"
                )

            if not 0.0 <= settings.analysis_temperature <= 2.0:
                errors.append(
                    f"Temperature must be between 0.0 and 2.0, got: {settings.analysis_temperature}"
                )

            if not 0.0 <= settings.analysis_confidence_cutoff <= 1.0:
                errors.append(
                    f"Confidence cutoff must be between 0.0 and 1.0, got: {settings.analysis_confidence_cutoff}"
                )

            # Validate processing configuration
            if settings.max_concurrent_requests <= 0:
                errors.append(
                    f"Max concurrent requests must be positive, got: {settings.max_concurrent_requests}"
                )

            if settings.request_timeout_seconds <= 0:
                errors.append(
                    f"Request timeout must be positive, got: {settings.request_timeout_seconds}"
                )

            # Validate quality configuration
            if settings.golden_dataset_path and not os.path.exists(
                settings.golden_dataset_path
            ):
                errors.append(
                    f"Golden dataset path does not exist: {settings.golden_dataset_path}"
                )

            if not 0.0 <= settings.schema_validation_rate_threshold <= 1.0:
                errors.append(
                    f"Schema validation rate threshold must be between 0.0 and 1.0, got: {settings.schema_validation_rate_threshold}"
                )

            if not 0.0 <= settings.rubric_score_threshold <= 5.0:
                errors.append(
                    f"Rubric score threshold must be between 0.0 and 5.0, got: {settings.rubric_score_threshold}"
                )

            # Validate cache configuration
            if settings.cache_ttl_seconds <= 0:
                errors.append(
                    f"Cache TTL must be positive, got: {settings.cache_ttl_seconds}"
                )

            if settings.cache_max_size <= 0:
                errors.append(
                    f"Cache max size must be positive, got: {settings.cache_max_size}"
                )

            # Validate API configuration
            if settings.rate_limit_requests_per_minute <= 0:
                errors.append(
                    f"Rate limit must be positive, got: {settings.rate_limit_requests_per_minute}"
                )

            # Validate OPA configuration
            if settings.opa_timeout_seconds <= 0:
                errors.append(
                    f"OPA timeout must be positive, got: {settings.opa_timeout_seconds}"
                )
    ...
    def reload_config(self):
        ...
        try:
            ...
            logger.error("Failed to reload analysis configuration: %s", e)
            raise
        ...
        try:
            ...
            if "model" in config_data:
                ...
                if "confidence_cutoff" in model_config:
                    settings.analysis_confidence_cutoff = model_config[
                        "confidence_cutoff"
                    ]
            ...
            if "processing" in config_data:
                ...
                if "max_concurrent_requests" in processing_config:
                    settings.max_concurrent_requests = processing_config[
                        "max_concurrent_requests"
                    ]
                if "request_timeout_seconds" in processing_config:
                    settings.request_timeout_seconds = processing_config[
            ...
            logger.info("Configuration imported from: %s", config_path)
    ...
    def setup_environment(self, environment: str):
        ...
        try:
            ...
            os.environ["ANALYSIS_MAX_CONCURRENT_REQUESTS"] = str(
                env_config["processing"]["max_concurrent_requests"]
            )
            os.environ["ANALYSIS_RATE_LIMIT_REQUESTS_PER_MINUTE"] = str(
                env_config["api"]["rate_limit_requests_per_minute"]
            )
            os.environ["ANALYSIS_LOGGING_LEVEL"] = env_config["monitoring"][
                "logging_level"
            ]
            ...
            logger.info("Environment setup complete for: %s", environment)

        except Exception as e:
            logger.error("Failed to setup environment %s: %s", environment, e)
            raise
