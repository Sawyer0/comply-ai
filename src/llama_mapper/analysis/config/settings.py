...
class AnalysisSettings(BaseSettings):
    ...
    analysis_model_path: str = Field(
        default="models/phi3-mini-3.8b",
        description="Path to the Phi-3 Mini model"
    )
    ...
    idempotency_cache_ttl_hours: int = Field(
        ...
        ge=1,
        le=168,
        description="Idempotency cache TTL in hours"
    )
    ...
    golden_dataset_path: Optional[str] = Field(
        default=None,
        description="Path to golden dataset for quality evaluation"
    )
    ...
    opa_binary_path: Optional[str] = Field(
        default=None,
        description="Path to OPA binary (defaults to 'opa' in PATH)"
    )
    opa_validation_enabled: bool = Field(
        default=True,
        description="Enable OPA policy validation"
    )
    ...
    circuit_breaker_enabled: bool = Field(
        default=True,
        description="Enable circuit breaker for model server calls"
    )
    ...
    schema_fallback_enabled: bool = Field(
        default=True,
        description="Enable schema validation fallback"
    )
...
class AnalysisConfig:
    ...
    def api(self) -> Dict[str, Any]:
        ...
        return {
            "host": getattr(self.base_settings, "host", "localhost") if self.base_settings else "localhost",
    ...
    def from_config_manager(cls, config_manager) -> "AnalysisConfig":
        ...
        if "model" in config_dict:
            ...
            analysis_settings_dict["analysis_model_path"] = model_config.get("name", "models/phi3-mini-3.8b")
            analysis_settings_dict["analysis_temperature"] = model_config.get("temperature", 0.1)
            analysis_settings_dict["analysis_confidence_cutoff"] = model_config.get("confidence_threshold", 0.3)
        
        # API settings
        if "api" in config_dict:
            api_config = config_dict["api"]
            analysis_settings_dict["max_concurrent_requests"] = api_config.get("max_concurrent_requests", 10)
            analysis_settings_dict["request_timeout_seconds"] = api_config.get("timeout_seconds", 30)
        
        # Quality settings
        if "quality" in config_dict:
            quality_config = config_dict["quality"]
            analysis_settings_dict["quality_evaluation_enabled"] = quality_config.get("evaluation_enabled", True)
