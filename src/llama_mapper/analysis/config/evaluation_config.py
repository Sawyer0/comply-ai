...
class EvaluationThresholds(BaseModel):
    ...
    rubric_score: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Minimum rubric score"
    )
    ...
    evidence_accuracy: float = Field(
        default=0.85, ge=0.0, le=1.0, description="Minimum evidence accuracy"
    )
...
class StorageConfig(BaseModel):
    ...
    backend_type: str = Field(default="file", description="Storage backend type")
...
class WeeklyEvaluationConfig(BaseModel):
    ...
    evaluation_period_days: int = Field(
        default=7, ge=1, le=30, description="Number of days to include in evaluation"
    )
    max_concurrent_evaluations: int = Field(
        default=5, ge=1, le=20, description="Maximum concurrent evaluations"
    )
    timeout_minutes: int = Field(
        default=30, ge=5, le=120, description="Evaluation timeout in minutes"
    )
    ...
    reports: ReportConfig = Field(
        default_factory=ReportConfig, description="Report generation configuration"
    )
    ...
    def from_environment(cls) -> "WeeklyEvaluationConfig":
        ...
        if os.getenv("LLAMA_MAPPER_WEEKLY_EVALUATIONS_ENABLED"):
            config_data["enabled"] = (
                os.getenv("LLAMA_MAPPER_WEEKLY_EVALUATIONS_ENABLED").lower() == "true"
            )

        if os.getenv("LLAMA_MAPPER_DEFAULT_WEEKLY_SCHEDULE"):
            config_data["default_schedule"] = os.getenv(
                "LLAMA_MAPPER_DEFAULT_WEEKLY_SCHEDULE"
            )

        if os.getenv("LLAMA_MAPPER_EVALUATION_PERIOD_DAYS"):
            config_data["evaluation_period_days"] = int(
                os.getenv("LLAMA_MAPPER_EVALUATION_PERIOD_DAYS")
            )

        # Thresholds
        thresholds = {}
        if os.getenv("LLAMA_MAPPER_SCHEMA_VALID_THRESHOLD"):
            thresholds["schema_valid_rate"] = float(
                os.getenv("LLAMA_MAPPER_SCHEMA_VALID_THRESHOLD")
            )
        if os.getenv("LLAMA_MAPPER_RUBRIC_SCORE_THRESHOLD"):
            thresholds["rubric_score"] = float(
                os.getenv("LLAMA_MAPPER_RUBRIC_SCORE_THRESHOLD")
            )
        if os.getenv("LLAMA_MAPPER_OPA_COMPILE_THRESHOLD"):
            thresholds["opa_compile_success_rate"] = float(
                os.getenv("LLAMA_MAPPER_OPA_COMPILE_THRESHOLD")
            )
        if os.getenv("LLAMA_MAPPER_EVIDENCE_ACCURACY_THRESHOLD"):
            thresholds["evidence_accuracy"] = float(
                os.getenv("LLAMA_MAPPER_EVIDENCE_ACCURACY_THRESHOLD")
            )
        ...
        if os.getenv("LLAMA_MAPPER_NOTIFICATION_EMAIL"):
            notifications["email_recipients"] = os.getenv(
                "LLAMA_MAPPER_NOTIFICATION_EMAIL"
            ).split(",")
        if os.getenv("SLACK_WEBHOOK_URL"):
            notifications["slack_webhook_url"] = os.getenv("SLACK_WEBHOOK_URL")
        if os.getenv("LLAMA_MAPPER_WEBHOOK_URLS"):
            notifications["webhook_urls"] = os.getenv(
                "LLAMA_MAPPER_WEBHOOK_URLS"
            ).split(",")
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if (
            self.enabled
            and not self.notifications.email_recipients  # pylint: disable=no-member
            and not self.notifications.slack_webhook_url  # pylint: disable=no-member
        ):
            issues.append("Notifications are enabled but no recipients configured")

        if (
            self.storage.backend_type == "file" and not self.storage.storage_dir
    ...
    def get_storage_backend_config(self) -> Dict[str, Any]:
        ...
        config = {
            "backend_type": self.storage.backend_type,  # pylint: disable=no-member
        }

        if self.storage.storage_dir:  # pylint: disable=no-member
            config["storage_dir"] = (
                self.storage.storage_dir
            )  # pylint: disable=no-member
        if self.storage.database_url:  # pylint: disable=no-member
            config["database_url"] = (
                self.storage.database_url
            )  # pylint: disable=no-member
        if self.storage.s3_bucket:  # pylint: disable=no-member
            config["s3_bucket"] = self.storage.s3_bucket  # pylint: disable=no-member
