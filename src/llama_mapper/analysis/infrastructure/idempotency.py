...
class IdempotencyManager(IIdempotencyManager):
    ...
    def __init__(self, ttl_hours: int = 24, max_items: int = 1000):
        ...
        logger.info(f"Initialized Idempotency Manager with TTL: {ttl_hours}h, Max items: {max_items}")
    ...
    def _generate_cache_key(self, idempotency_key: str, request_data: Dict[str, Any]) -> str:
        """
        Generate cache key from idempotency key and request data.
...
class RedisIdempotencyManager(IdempotencyManager):
    ...
    async def get_cached_response(
        ...
        try:
            ...
            logger.error(f"Error retrieving cached response from Redis: {e}")
            return None
    ...
    async def cache_response(
        ...
        try:
            ...
            logger.error(f"Error caching response in Redis: {e}")
    ...
    async def cleanup_expired_entries(self) -> int:
        ...
        try:
            ...
            logger.error(f"Error cleaning up expired entries in Redis: {e}")
            return 0