...
class APIKeyRequest(BaseModel):
    ...
    expires_in_days: Optional[int] = Field(
        None, description="Expiration in days (None for no expiration)"
    )
...
class APIKeyManager:
    ...
    def _store_api_key(self, api_key: APIKey) -> bool:
        ...
        try:
            ...
            logger.error("Failed to store API key %s: %s", api_key.key_id, e)
            return False
    ...
    def _load_api_key(self, key_id: str) -> Optional[APIKey]:
        ...
        try:
            ...
            logger.error("Failed to load API key %s: %s", key_id, e)
            return None
    ...
    def create_api_key(self, request: APIKeyRequest) -> Optional[APIKeyResponse]:
        ...
        try:
            ...
            logger.info("Created API key %s for tenant %s", key_id, request.tenant_id)
    ...
    def validate_api_key(
        self, api_key: str, required_scopes: List[APIKeyScope]
    ) -> Optional[APIKey]:
        """
        ...
        try:
            ...
            if stored_key.status != APIKeyStatus.ACTIVE:
                logger.warning(
                    f"API key {stored_key.key_id} is not active: {stored_key.status}"
                )
                return None
            ...
            if stored_key.expires_at and stored_key.expires_at < datetime.now(
                ...
                logger.warning("API key %s has expired", stored_key.key_id)
                # Mark as expired
            ...
            if not all(scope in stored_key.scopes for scope in required_scopes):
                logger.warning("API key %s lacks required scopes", stored_key.key_id)
                return None
    ...
    def rotate_api_key(self, key_id: str) -> Optional[APIKeyResponse]:
        ...
        try:
            ...
            logger.info("Rotated API key %s -> %s", key_id, new_key_id)
            ...
            logger.error("Failed to rotate API key %s: %s", key_id, e)
            return None
    ...
    def list_tenant_keys(self, tenant_id: str) -> List[APIKey]:
        ...
        try:
            ...
            logger.error("Failed to list keys for tenant %s: %s", tenant_id, e)
            return []
    ...
    def cleanup_expired_keys(self) -> int:
        ...
        try:
            ...
            if self.redis_client:
                ...
                for key_id in self.redis_client.scan_iter(match=f"{self.key_prefix}*"):
                    api_key = self._load_api_key(
                        key_id.decode().replace(self.key_prefix, "")
                    )
                    if (
                        api_key
                        and api_key.expires_at
                        and api_key.expires_at < current_time
                    ):
            ...
            if cleaned_count > 0:
                logger.info("Cleaned up %s expired API keys", cleaned_count)

            return cleaned_count

        except Exception as e:
            logger.error("Failed to cleanup expired keys: %s", e)
            return 0
