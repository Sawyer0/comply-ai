...
class Phi3AnalysisModelServer(IModelServer):
    ...
    def __init__(
        ...
        self._version_info = VersionInfo(
            taxonomy="v1.0.0",
            frameworks="v1.0.0", 
            analyst_model="phi3-mini-v1.0.0"
        )
        
        logger.info(f"Initialized Phi-3 Analysis Model Server with model: {model_path}")
    ...
    async def analyze(self, request: AnalysisRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if confidence < self.confidence_cutoff:
                ...
                analysis_output["_fallback_reason"] = f"Low confidence: {confidence:.3f} < {self.confidence_cutoff}"
...
"""
    ...
    async def _generate_analysis(self, prompt: str, request: AnalysisRequest) -> Dict[str, Any]:
        """
    ...
    def _compute_confidence(self, analysis_output: Dict[str, Any], request: AnalysisRequest) -> float:
        """
