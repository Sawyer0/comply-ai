...
class AnalysisValidator(IValidator):
    ...
    def __init__(self, schema_path: str = "schemas/analyst_output.json"):
        ...
        logger.info("Initialized Analysis Validator with schema: %s", schema_path)
    ...
    def validate_and_fallback(
        ...
        try:
            ...
            if not is_valid:
                logger.warning("Model output failed schema validation: %s", errors)
                # Return template fallback
                return self._get_template_fallback(
                    request, f"Schema validation failed: {', '.join(errors)}"
                )
    ...
    def validate_schema_compliance(self, output: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error("Schema compliance check error: %s", e)
            return False
    ...
    def _get_template_fallback(
        self, request: AnalysisRequest, reason: str
    ) -> Dict[str, Any]:
        """
    ...
    def _get_coverage_gap_template(
        ...
        # Truncate reason to ensure notes field stays under 500 chars
        ...
        return {
            ...
            "notes": f"Template fallback due to validation error",
            "_template_fallback": True,
            "_fallback_reason": truncated_reason,
        }
    ...
    def _get_false_positive_template(
        ...
        return {
            ...
            "notes": "Template fallback due to validation error",
            "_template_fallback": True,
            "_fallback_reason": reason[:400] if len(reason) > 400 else reason,
        }
