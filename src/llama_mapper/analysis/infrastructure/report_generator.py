...
class WeeklyEvaluationReportGenerator(IReportGenerator):
    ...
    def _generate_pdf_report(
        ...
        try:
            ...
            metrics_data = [
                ...
                ("Schema Validation Rate", f"{quality_metrics.get('schema_valid_rate', 0):.2%}"),
                ("Rubric Score", f"{quality_metrics.get('rubric_score', 0):.2f}"),
                ("OPA Compilation Success Rate", f"{quality_metrics.get('opa_compile_success_rate', 0):.2%}"),
                ("Evidence Accuracy", f"{quality_metrics.get('evidence_accuracy', 0):.2f}"),
            ]
            ...
            if individual_scores:
                ...
                story.append(Paragraph(f"Min Score: {min(individual_scores):.2f}", styles['Normal']))
                story.append(Paragraph(f"Max Score: {max(individual_scores):.2f}", styles['Normal']))
                story.append(Spacer(1, 12))
            ...
            logger.info(f"Generated PDF report for tenant {tenant_id}")
            return buffer.getvalue()
            
        except ImportError:
            logger.error("reportlab not available for PDF generation")
            # Fallback to simple text report
            return self._generate_text_report(quality_metrics, tenant_id, requested_by, report_type).encode('utf-8')
        except Exception as e:
    ...
    def _generate_csv_report(
        ...
        try:
            ...
            # Write header
            writer.writerow([
                "Metric", "Value", "Tenant ID", "Generated At", "Requested By", "Report Type"
            ])
            ...
            metrics_data = [
                ...
                ("opa_compile_success_rate", quality_metrics.get("opa_compile_success_rate", 0)),
            ...
            logger.info(f"Generated CSV report for tenant {tenant_id}")
            return buffer.getvalue()
    ...
    def _generate_json_report(
        ...
        try:
            report = {
                ...
                "summary": {
                    "total_examples": quality_metrics.get("total_examples", 0),
                    "average_rubric_score": sum(quality_metrics.get("individual_rubric_scores", [])) / max(len(quality_metrics.get("individual_rubric_scores", [])), 1),
                    "schema_validation_rate": quality_metrics.get("schema_valid_rate", 0),
                    "opa_compilation_success_rate": quality_metrics.get("opa_compile_success_rate", 0),
                    "evidence_accuracy": quality_metrics.get("evidence_accuracy", 0),
                },
            ...
            logger.info(f"Generated JSON report for tenant {tenant_id}")
            return report
    ...
    def _generate_alerts(self, quality_metrics: Dict[str, Any]) -> List[str]:
        ...
        if schema_rate < 0.98:
            alerts.append(f"Schema validation rate ({schema_rate:.2%}) below threshold (98%)")
        ...
        if opa_rate < 0.95:
            alerts.append(f"OPA compilation success rate ({opa_rate:.2%}) below threshold (95%)")
        
        evidence_accuracy = quality_metrics.get("evidence_accuracy", 0)
        if evidence_accuracy < 0.85:
            alerts.append(f"Evidence accuracy ({evidence_accuracy:.2f}) below threshold (0.85)")
    ...
    def _generate_recommendations(self, quality_metrics: Dict[str, Any]) -> List[str]:
        ...
        schema_rate = quality_metrics.get("schema_valid_rate", 0)
        if schema_rate < 0.98:
            recommendations.append("Review schema validation logic and improve data quality")
        
        rubric_score = quality_metrics.get("rubric_score", 0)
        if rubric_score < 0.8:
            recommendations.append("Consider retraining the model or adjusting evaluation criteria")
        
        opa_rate = quality_metrics.get("opa_compile_success_rate", 0)
        if opa_rate < 0.95:
            recommendations.append("Review OPA policy generation and fix compilation issues")
