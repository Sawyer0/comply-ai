    ...
    def validate_response_security(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for field in text_fields:
                if field in redacted_response and isinstance(
                    redacted_response[field], str
                ):
                    redacted_response[field] = self._redact_pii(
                        redacted_response[field]
                    )

            # Redact PII from OPA diff if present
            if "opa_diff" in redacted_response and isinstance(
                redacted_response["opa_diff"], str
            ):
                redacted_response["opa_diff"] = self._redact_pii(
                    redacted_response["opa_diff"]
                )

            # Redact PII from evidence references
            if "evidence_refs" in redacted_response and isinstance(
                redacted_response["evidence_refs"], list
            ):
                redacted_response["evidence_refs"] = [
                    self._redact_pii(str(ref))
                    for ref in redacted_response["evidence_refs"]
                ]
        ...
        try:
            ...
            for pattern in injection_patterns:
                if re.search(pattern, request_str, re.IGNORECASE):
                    logger.warning("Potential injection attempt detected: %s", pattern)
                    return False
            ...
            for path in suspicious_paths:
                if path in request_str:
                    logger.warning("Suspicious path detected: %s", path)
                    return False

            return True

        except Exception as e:
            logger.error("Request security validation error: %s", e)
            return False
