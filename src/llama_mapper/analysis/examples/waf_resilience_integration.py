...
from ..security.waf import WAFFactory, WAFMiddleware, WAFRuleEngine
...
class AnalysisModuleWithSecurity:
    ...
    def _setup_resilience(self):
        ...
        self.model_circuit_breaker = self.resilience_manager.create_circuit_breaker(
            name="model_server", config=circuit_breaker_config
        )
    ...
    async def start(self):
        ...
        logger.info("WAF Rules loaded: %s", len(self.waf_engine.rules))
        logger.info("Circuit Breaker State: %s", self.model_circuit_breaker.state.value)
...
async def main():
    ...
    logger.info("Total requests: %s", waf_stats["total_requests"])
    logger.info("Blocked requests: %s", waf_stats["blocked_requests"])

    logger.info("=== Resilience Statistics ===")
    resilience_stats = analysis_module.resilience_manager.get_all_statistics()
    logger.info(
        f"Circuit breaker state: {resilience_stats['circuit_breakers']['model_server']['state']}"
    )
