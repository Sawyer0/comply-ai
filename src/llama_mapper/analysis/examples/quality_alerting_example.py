...
from ..quality import (
    ...
    QualityAlertingSettings, AlertSeverity
)
...
class QualityAlertingExample:
    ...
    def add_custom_thresholds(self):
        ...
        self.alerting_system.add_threshold(QualityThreshold(
    ...
    async def simulate_normal_operation(self):
        ...
        for i in range(20):
            ...
            self.alerting_system.process_metric(QualityMetric(
    ...
    def show_system_status(self):
        ...
        logger.info(f"Thresholds Configured: {status['thresholds_configured']}")
        logger.info(f"Alert Handlers: {status['alert_handlers']}")
        logger.info(f"Metrics Processed: {status['statistics']['metrics_processed']}")
        logger.info(f"Degradations Detected: {status['statistics']['degradations_detected']}")
        logger.info(f"Alerts Created: {status['statistics']['alerts_created']}")
        ...
        for metric_type, value in dashboard_data['current_metrics'].items():
            logger.info(f"{metric_type}: {value:.3f}")
        
        logger.info("=== Quality Trends ===")
        for metric_type, trend in dashboard_data['trends'].items():
            logger.info(f"{metric_type}: {trend['trend']} (slope: {trend['slope']:.4f})")
