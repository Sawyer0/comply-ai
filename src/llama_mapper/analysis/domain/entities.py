...
class EvidenceReference(BaseModel):
    ...
    source: str = Field(
        description="Source of the evidence (e.g., 'detector_output', 'coverage_metrics')"
    )
...
class AnalysisRequest(BaseModel):
    ...
    required_detectors: List[str] = Field(
        min_length=1, max_length=20, description="List of required detector names"
    )
    ...
    high_sev_hits: List[Dict[str, Any]] = Field(
        max_length=100, description="High severity hits with taxonomy and scores"
    )
    false_positive_bands: List[Dict[str, Any]] = Field(
        max_length=50, description="False positive analysis bands"
    )
    policy_bundle: str = Field(
        pattern=r"^[a-zA-Z0-9\-\.]+$", description="Policy bundle identifier"
    )
    ...
    def validate_coverage_values(cls, v):
        ...
        for key, value in v.items():
            if not 0.0 <= value <= 1.0:
                raise ValueError(
                    f"Coverage value {value} for {key} must be between 0.0 and 1.0"
                )
        return v
...
class AnalysisResponse(BaseModel):
    ...
    evidence_refs: List[str] = Field(
        min_length=1, description="References to source metrics used"
    )
...
class BatchAnalysisRequest(BaseModel):
    ...
    requests: List[AnalysisRequest] = Field(
        min_length=1, max_length=100, description="List of analysis requests"
    )
...
class AnalysisErrorResponse(BaseModel):
    ...
    template_response: Optional[AnalysisResponse] = Field(
        default=None, description="Template response if fallback was used"
    )
...
class QualityMetrics(BaseModel):
    ...
    schema_valid_rate: float = Field(
        ge=0.0, le=1.0, description="Schema validation success rate"
    )
    rubric_score: float = Field(ge=0.0, le=5.0, description="Average rubric score")
    opa_compile_success_rate: float = Field(
        ge=0.0, le=1.0, description="OPA compilation success rate"
    )
    evidence_accuracy: float = Field(
        ge=0.0, le=1.0, description="Evidence reference accuracy"
    )
    individual_rubric_scores: List[float] = Field(
        description="Individual rubric scores"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
