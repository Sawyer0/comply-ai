...
class AnalysisService:
    ...
    async def analyze(self, request: AnalysisRequest) -> AnalysisResponse:
        ...
        try:
            ...
            validated_output = self.validator.validate_and_fallback(model_output, request)
            
            # Check if fallback was used
            if validated_output.get("_template_fallback"):
                # Create error response with template fallback
                return AnalysisErrorResponse(
                    error_type="confidence_fallback",
                    message=validated_output.get("_fallback_reason", "Low confidence fallback"),
                    request_id=request.request_id,
                    fallback_used=True,
                    mode="fallback",
                    template_response=self._create_response_from_output(validated_output, request)
                )
            ...
            redacted_dict = self.security_validator.validate_response_security(response_dict)
            
            return AnalysisResponse(**redacted_dict)
            
        except Exception as e:
            logger.error(f"Analysis service error: {e}")
            # Return error response with template fallback
            ...
            return AnalysisErrorResponse(
                ...
                fallback_used=True,
                mode="fallback",
                template_response=self._create_response_from_output(template_response, request)
            )
...
class BatchAnalysisService:
    ...
    async def analyze_batch(
        ...
        batch_response = BatchAnalysisResponse(
            ...
            error_count=sum(1 for r in responses if isinstance(r, AnalysisErrorResponse))
        )
    ...
    async def _process_requests_concurrently(
        ...
        async def process_single_request(request: AnalysisRequest) -> Union[AnalysisResponse, AnalysisErrorResponse]:
    ...
    async def _process_single_request(
        ...
        try:
            ...
            response = await asyncio.wait_for(
                self.analysis_service.analyze(request),
            ...
            logger.error(f"Request timeout for request: {request.request_id}")
            return AnalysisErrorResponse(
...
class WeeklyEvaluationService:
    ...
    async def schedule_weekly_evaluation(
        ...
        try:
            ...
            logger.error(f"Failed to schedule weekly evaluation for tenant {tenant_id}: {e}")
    ...
    async def run_scheduled_evaluation(self, schedule_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"Running scheduled evaluation for tenant {tenant_id}")
        ...
        try:
            ...
            if not recent_data:
                logger.warning(f"No recent data found for tenant {tenant_id}, using empty dataset")
            ...
            if schedule.get("report_recipients"):
                try:
                    ...
                    logger.error(f"Failed to send notifications for evaluation {schedule_id}: {notification_error}")
            ...
            self._stats["notifications_sent"] += len(schedule.get("report_recipients", []))
            ...
            logger.error(f"Failed to run scheduled evaluation {schedule_id}: {e}")
            ...
            try:
                ...
                logger.error(f"Failed to store failure result for evaluation {schedule_id}: {storage_error}")
    ...
    async def _generate_evaluation_report(
        ...
        report_path = await self.storage_backend.save_evaluation_report(
            tenant_id, report_content, "weekly_evaluation"
        )
    ...
    async def list_scheduled_evaluations(self, tenant_id: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def cancel_scheduled_evaluation(self, schedule_id: str) -> bool:
        ...
        logger.info(f"Cancelled scheduled evaluation {schedule_id}")
        return True
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_evaluation": self._stats["last_evaluation"].isoformat() if self._stats["last_evaluation"] else None,
            "active_schedules": len(self.scheduled_evaluations),
            "success_rate": (
                self._stats["evaluations_run"] / (self._stats["evaluations_run"] + self._stats["evaluations_failed"])
                if (self._stats["evaluations_run"] + self._stats["evaluations_failed"]) > 0
                else 0.0
            ),
        }
...
class HealthService:
    ...
    async def check_health(self) -> HealthStatus:
        ...
        try:
            ...
            checks["model_server"] = {"status": "healthy", "details": "Model server operational"}
        except Exception as e:
        ...
        try:
            ...
            checks["idempotency_manager"] = {"status": "healthy", "details": "Idempotency manager operational"}
        except Exception as e:
        ...
        try:
            ...
            checks["quality_evaluator"] = {"status": "healthy", "details": "Quality evaluator operational"}
        except Exception as e:
