...
class RetryManager(IRetryManager):
    ...
    def __init__(
        ...
        logger.info(f"Retry manager initialized with config: {self._config}")
        ...
        for attempt in range(1, self._config.max_attempts + 1):
            ...
            try:
                ...
                if not self._should_retry(e, attempt):
                    logger.warning(f"Function {func.__name__} failed with non-retryable exception: {e}")
                    raise
    ...
    def _calculate_delay(self, attempt: int) -> float:
        ...
        elif self._config.strategy == RetryStrategy.FIXED_DELAY:
