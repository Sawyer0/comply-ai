        ...
        for pii_type, pattern in self.pii_patterns.items():
            if pii_type == "email":
                scrubbed_text = re.sub(
                    pattern, "[EMAIL_REDACTED]", scrubbed_text, flags=re.IGNORECASE
                )
            elif pii_type == "phone":
                ...
                for phone_pattern in phone_patterns:
                    scrubbed_text = re.sub(
                        phone_pattern, "[PHONE_REDACTED]", scrubbed_text
                    )
            elif pii_type == "ssn":
                ...
                scrubbed_text = re.sub(
                    api_key_pattern, "[API_KEY_REDACTED]", scrubbed_text
                )
                ...
                if "[API_KEY_REDACTED]" not in scrubbed_text:
                    scrubbed_text = re.sub(
                        pattern, "[KEY_REDACTED]", scrubbed_text, flags=re.IGNORECASE
                    )
...
class LogScrubbingMiddleware(BaseHTTPMiddleware):
    ...
    async def _extract_response_info(self, response: Response) -> Dict[str, Any]:
        ...
        return {"headers": scrubbed_headers, "size": response_size}
...
class RequestResponseLogger:
    ...
    def log_request(
        ...
        """
        log_data = {"request_id": request_id, "method": method, "path": path}
    ...
    def log_response(
        ...
        """
        log_data = {"request_id": request_id, "status_code": status_code}
