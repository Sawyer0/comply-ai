...
class AnalysisMiddleware:
    ...
    def metrics_middleware() -> Callable:
        ...
        async def middleware(request: Request, call_next: Callable) -> Response:
            ...
            logger.debug(f"Request processed in {processing_time:.2f}ms: {request.method} {request.url.path}")
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if client_ip in self.request_counts:
            recent_requests = [ts for ts in self.request_counts[client_ip] if ts > current_time - 60]
