...
def create_analysis_app(
    ...
    app.dependency_overrides[AnalysisServiceDep] = (
        lambda: factory.get_analysis_service()
    )
    app.dependency_overrides[BatchAnalysisServiceDep] = (
        lambda: factory.get_batch_analysis_service()
    )
...
def create_analysis_app_with_config(
    ...
    if config_path:
        ...
        logger.info("Loading settings from config file: %s", config_path)
