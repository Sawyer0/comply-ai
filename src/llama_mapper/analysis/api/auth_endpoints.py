...
class APIKeyUpdateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="New description for the API key"
    )
    scopes: Optional[List[str]] = Field(None, description="New scopes for the API key")
    rate_limit: Optional[int] = Field(
        None, description="New rate limit for the API key"
    )
    ...
    def _register_endpoints(self):
        ...
        async def create_api_key(
            ...
            try:
                ...
                logger.info(
                    f"Created API key {response.key_id} for tenant {request.tenant_id}"
                )
                return response
        ...
        async def rotate_api_key(
            ...
            try:
                ...
                logger.info("Rotated API key %s -> %s", request.key_id, response.key_id)
                return response

            except HTTPException:
                raise
            except Exception as e:
                logger.error("Error rotating API key: %s", e)
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Internal server error",
                )
        ...
        async def revoke_api_key(
            ...
            try:
                ...
                logger.info("Revoked API key %s", request.key_id)
