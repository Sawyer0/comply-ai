...
class AnalysisMetricsCollector(MetricsCollector):
    ...
    def record_analysis_request(
        ...
        self.counter("analysis_requests_total", labels).inc()
        self.histogram("analysis_request_duration_ms", labels).observe(processing_time)
        
        logger.debug(f"Recorded analysis request: {analysis_type}, {processing_time}ms, success={success}")
    ...
    def record_schema_validation(
        ...
        total_validations = self.counter("schema_validation_total", {"success": "true"})._value + \
                          self.counter("schema_validation_total", {"success": "false"})._value
        if total_validations > 0:
            success_rate = self.counter("schema_validation_total", {"success": "true"})._value / total_validations
            self.gauge("schema_valid_rate", {}).set(success_rate)
        
        logger.debug(f"Recorded schema validation: success={success}, fallback={fallback_used}")
    ...
    def record_template_fallback(
        ...
        if total_requests > 0:
            fallback_rate = self.counter("template_fallback_total", {})._value / total_requests
            self.gauge("template_fallback_rate", {}).set(fallback_rate)
        
        logger.debug(f"Recorded template fallback: {fallback_reason}")
    ...
    def record_opa_compilation(
        ...
        total_compilations = self.counter("opa_compilation_total", {"success": "true"})._value + \
                           self.counter("opa_compilation_total", {"success": "false"})._value
        if total_compilations > 0:
            success_rate = self.counter("opa_compilation_total", {"success": "true"})._value / total_compilations
            self.gauge("opa_compile_success_rate", {}).set(success_rate)
        
        logger.debug(f"Recorded OPA compilation: success={success}")
    
    def record_quality_score(
        self, 
        score: float,
        evaluation_type: str = "rubric"
    ):
        """
        ...
        logger.debug(f"Recorded quality score: {score} ({evaluation_type})")
    ...
    def record_batch_processing(
        ...
        logger.debug(f"Recorded batch processing: size={batch_size}, success={success_count}, failures={failure_count}")
    ...
    def end_request_timer(self, request_id: str) -> Optional[float]:
        ...
        if request_id not in self._request_start_times:
            logger.warning(f"Request timer not found for ID: {request_id}")
            return None
    ...
    def get_analysis_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "opa_compile_success_rate": self.gauge("opa_compile_success_rate", {})._value,
            "active_timers": len(self._request_start_times)
        }

    def record_waf_scan(self, is_safe: bool, violations_count: int, processing_time_ms: float, client_ip: str = "unknown"):
        """
        ...
        if violations_count > 0:
            self.counter("waf_violations_total", {
                "client_ip": client_ip[:8] + "..." if len(client_ip) > 8 else client_ip
            }).inc()
        
        logger.debug(f"Recorded WAF scan: safe={is_safe}, violations={violations_count}, duration={processing_time_ms}ms")

    def record_waf_violation(self, violation_type: str, severity: str, rule_name: str, client_ip: str = "unknown"):
        """
        ...
        Args:
            ...
            client_ip: Client IP address.
        """
        self.counter("waf_violations_by_type_total", {
            "type": violation_type,
            "severity": severity,
            "rule": rule_name,
            "client_ip": client_ip[:8] + "..." if len(client_ip) > 8 else client_ip
        ...
        logger.warning(f"Recorded WAF violation: type={violation_type}, severity={severity}, rule={rule_name}")
    ...
    def record_circuit_breaker_state(self, state: str, service: str):
        ...
        logger.info(f"Circuit breaker state changed: {service} -> {state}")
    ...
    def record_retry_attempt(self, service: str, attempt: int, success: bool):
        ...
        """
        self.counter("retry_attempts_total", {
            "service": service,
            "attempt": str(attempt),
            "success": str(success)
        }).inc()
        
        logger.debug(f"Recorded retry attempt: {service} attempt={attempt} success={success}")
