...
class WAFMiddleware(BaseHTTPMiddleware, IWAFMiddleware):
    ...
    async def _handle_violations(
        self, violations: list[WAFViolation], client_ip: str, request: Request
    ) -> None:
    ...
    def _log_violations(
        ...
        for violation in violations:
            ...
            if violation.severity.value == "critical":
                logger.critical("WAF Critical Violation: %s", log_data)
            elif violation.severity.value == "high":
                logger.error("WAF High Severity Violation: %s", log_data)
            elif violation.severity.value == "medium":
                logger.warning("WAF Medium Severity Violation: %s", log_data)
            else:
                logger.info("WAF Low Severity Violation: %s", log_data)
