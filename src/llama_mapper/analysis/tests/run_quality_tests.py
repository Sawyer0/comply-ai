...
def run_tests(
    ...
    if coverage:
        cmd.extend(["--cov=src.llama_mapper.analysis.quality", "--cov-report=html", "--cov-report=term"])
...
def run_linting() -> Dict[str, Any]:
    ...
    # Run flake8
    flake8_cmd = ["flake8", str(quality_dir), "--max-line-length=88", "--extend-ignore=E203,W503"]
