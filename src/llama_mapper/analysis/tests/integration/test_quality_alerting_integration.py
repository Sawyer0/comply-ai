...
class TestQualityAlertingIntegration:
    ...
    def test_metric_processing_and_alerting(self, alerting_system):
        ...
        try:
            ...
            assert (
                alert.degradation_detection.degradation_type
                == DegradationType.THRESHOLD_BREACH
            )
    ...
    def test_multiple_alert_handlers(
        self, alerting_system, mock_email_handler, mock_slack_handler
    ):
    ...
    def test_anomaly_detection(self, alerting_system):
        ...
        anomalies = alerting_system.degradation_detector.detect_anomalies(
            QualityMetricType.SCHEMA_VALIDATION_RATE, all_metrics
        )
    ...
    def test_dashboard_data_generation(self, alerting_system):
        ...
        # Check specific data
        assert (
            QualityMetricType.SCHEMA_VALIDATION_RATE.value
            in dashboard_data["current_metrics"]
        )
        assert (
            QualityMetricType.SCHEMA_VALIDATION_RATE.value in dashboard_data["trends"]
        )
    ...
    def test_configuration_validation(self):
        ...
        with pytest.raises(ValueError):
            QualityAlertingSystem(
                monitoring_interval_seconds=-1, max_metrics_per_type=1000  # Invalid
            )

        with pytest.raises(ValueError):
            QualityAlertingSystem(
                monitoring_interval_seconds=60, max_metrics_per_type=0  # Invalid
            )
...
class TestQualityAlertingSystemWithRealHandlers:
    ...
    def alerting_system_with_handlers(self):
        ...
        from ...quality import LoggingAlertHandler
    ...
    def test_logging_handler_integration(self, alerting_system_with_handlers):
        ...
        with patch(
            "llama_mapper.analysis.quality.alerting.alert_handlers.logger"
        ) as mock_logger:
            ...
            active_alerts = (
                alerting_system_with_handlers.alert_manager.get_active_alerts()
            )
            if active_alerts:
