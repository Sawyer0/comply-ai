...
class TestPhi3AnalysisModelServer:
    ...
    def model_server(self):
        ...
        return Phi3AnalysisModelServer(
            model_path="test-model-path", temperature=0.1, confidence_cutoff=0.3
        )
    ...
    def test_determine_analysis_type_incident_summary(
        self, model_server, sample_request
    ):
    ...
    def test_determine_analysis_type_insufficient_data(
        self, model_server, sample_request
    ):
    ...
    def test_compute_confidence_with_evidence(self, model_server):
        ...
        confidence_without = model_server._compute_confidence(
            test_data_without_evidence
        )
    ...
    def test_simulate_phi3_inference_analysis_type_specific(
        self, model_server, sample_request
    ):
        ...
        result1 = model_server._simulate_phi3_inference(
            "coverage gap prompt", sample_request
        )

        # Test false positive
        sample_request.false_positive_bands = [{"band": "high"}]
        result2 = model_server._simulate_phi3_inference(
            "false positive prompt", sample_request
        )
