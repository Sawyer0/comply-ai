...
class CheckpointManager:
    ...
    def _generate_version_id(self, major: int = 1, minor: int = 0, patch: int = 0) -> str:
        ...
        return f"{self.version_prefix}@v{new_version[0]}.{new_version[1]}.{new_version[2]}"
    ...
    def save_checkpoint(
        ...
        try:
            ...
            full_metadata = {
                ...
                "peft_config": model.peft_config if hasattr(model, 'peft_config') else {},
                "tags": tags or [],
            ...
            logger.info(
                ...
                version_id=version_id,
                checkpoint_size_mb=self._get_directory_size(checkpoint_dir) / (1024 * 1024),
            )
    ...
    def load_checkpoint(
        ...
        try:
            ...
            if base_model is not None:
                ...
                model = AutoPeftModelForCausalLM.from_pretrained(checkpoint_dir / "model")
    ...
    def get_latest_version(self, tags: Optional[List[str]] = None) -> Optional[ModelVersion]:
        """
    ...
    def rollback_to_version(self, version_id: str) -> Tuple[PeftModel, PreTrainedTokenizer, Dict[str, Any]]:
        """
        ...
        rollback_metadata = {
            **metadata,
            "rollback_from": self.get_latest_version().version if self.get_latest_version() else None,
    ...
    def get_deployment_info(self, version_id: str) -> Dict[str, Any]:
        ...
        deployment_info = {
            ...
            "model_size_mb": self._get_directory_size(Path(version.checkpoint_path)) / (1024 * 1024),
