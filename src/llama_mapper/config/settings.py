...
class ModelConfig(BaseModel):
    ...
    quantization: Optional[str] = Field(
        default=None, description="Quantization method (8bit, 4bit)"
    )
...
class TrainingConfig(BaseModel):
    ...
    num_epochs: int = Field(
        default=2, ge=1, le=10, description="Number of training epochs"
    )
    batch_size: int = Field(default=4, ge=1, le=32, description="Training batch size")
    gradient_accumulation_steps: int = Field(
        default=4, ge=1, le=16, description="Gradient accumulation steps"
    )
    max_seq_length: int = Field(
        default=1024, ge=128, le=2048, description="Maximum sequence length"
    )
    warmup_steps: int = Field(default=100, ge=0, le=1000, description="Warmup steps")
    save_steps: int = Field(
        default=500, ge=100, le=2000, description="Save checkpoint every N steps"
    )
...
class ConfidenceConfig(BaseModel):
    ...
    default_threshold: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Default confidence threshold"
    )
    detector_thresholds: Dict[str, float] = Field(
        default_factory=dict, description="Per-detector confidence thresholds"
    )
...
class ServingConfig(BaseModel):
    ...
    max_batch_size: int = Field(
        default=32, ge=1, le=128, description="Maximum batch size"
    )
...
class StorageConfig(BaseModel):
    ...
    storage_backend: str = Field(
        default="postgresql", description="Storage backend (postgresql, clickhouse)"
    )
...
class LoggingConfig(BaseModel):
    ...
    max_message_length: int = Field(
        default=500, ge=100, le=2000, description="Maximum log message length"
    )
...
class SecurityConfig(BaseModel):
    ...
    secrets_backend: str = Field(
        default="vault", description="Secrets backend (vault, aws)"
    )
...
class Settings(BaseSettings):
    ...
    taxonomy_path: str = Field(
        default="pillars-detectors/taxonomy.yaml", description="Path to taxonomy YAML"
    )
    detectors_path: str = Field(
        default="pillars-detectors", description="Path to detector configs directory"
    )
    frameworks_path: str = Field(
        default="pillars-detectors/frameworks.yaml",
        description="Path to frameworks YAML",
    )
    schema_path: str = Field(
        default="pillars-detectors/schema.json",
        description="Path to output schema JSON",
    )
    ...
    def __init__(self, **values: object) -> None:  # type: ignore[override]
        # Allow nested kwarg style e.g. storage__storage_backend="clickhouse"
        processed: dict[str, object] = {}
        nested: dict[str, dict[str, object]] = {}
        for k, v in values.items():
            if isinstance(k, str) and "__" in k:
                root, sub = k.split("__", 1)
                nested.setdefault(root, {})[sub] = v
            else:
                processed[k] = v
        # Merge nested dicts so pydantic can coerce them into submodels
        processed.update(nested)
        super().__init__(**processed)  # type: ignore[arg-type]
