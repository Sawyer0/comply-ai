...
def _register_legacy_commands(registry) -> None:
    ...
    pipeline.register(temp_main)
    quality.register(temp_main)
    security.register(temp_main)
                ...
                def execute(self, **kwargs):
                    ...
                    try:
                        ...
                        help_text = getattr(
                            self.click_command,
                            "help",
                            f"Legacy {self.click_command.name} command",
                        )
                        click.echo(f"Legacy command: {self.click_command.name}")
                        click.echo(f"Help: {help_text}")
                        click.echo(
                            "Note: This command needs to be converted to the new registry system"
                        )
