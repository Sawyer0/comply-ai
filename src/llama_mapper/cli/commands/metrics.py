                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    ...
                    if fmt == "prometheus":
                        ...
                        if metrics_response.status_code == 200:
                            ...
                            raise Exception(
                                f"Failed to get Prometheus metrics: HTTP {metrics_response.status_code}"
                            )
                    else:
                        ...
                        if metrics_response.status_code == 200:
                            ...
                            if fmt == "yaml":
                                ...
                                metrics_data = yaml.dump(
                                    metrics_data, default_flow_style=False
                                )
                            else:  # json
                                metrics_data = json.dumps(
                                    metrics_data, indent=2, default=str
                                )
                        else:
                            raise Exception(
                                f"Failed to get metrics summary: HTTP {metrics_response.status_code}"
                            )
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    ...
                    if metrics_response.status_code == 200:
                        ...
                        raise Exception(
                            f"Failed to get metrics: HTTP {metrics_response.status_code}"
                        )
                    ...
                    dashboard_data = {
                        ...
                        "status": (
                            "healthy" if not alerts_data.get("alerts") else "degraded"
                        ),
                    }
...
def _analyze_profile_data(profile_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis["response_time_stats"] = {
            ...
            "trend": (
                ...
                if response_times[-1] > response_times[0]
                    ...
                    "decreasing" if response_times[-1] < response_times[0] else "stable"
                )
            ),
        }
    ...
    if request_rates:
        analysis["throughput_stats"] = {
            ...
            "trend": (
                "increasing"
                if request_rates[-1] > request_rates[0]
                else "decreasing" if request_rates[-1] < request_rates[0] else "stable"
            ),
        }
    ...
    if error_rates:
        analysis["error_stats"] = {
            ...
            "trend": (
                ...
                else "decreasing" if error_rates[-1] < error_rates[0] else "stable"
            ),
        }
