...
class SystemMetricsCommand(AsyncCommand):
    ...
    async def execute_async(self, **kwargs: Any) -> None:
        ...
        if refresh_interval > 0:
            await self._monitor_continuous(
                refresh_interval, output_format, include_detailed
            )
        else:
    ...
    def _get_process_metrics(self, include_detailed: bool) -> Dict[str, Any]:
        ...
        if include_detailed:
            ...
            top_cpu = sorted(
                processes, key=lambda p: p.info.get("cpu_percent", 0), reverse=True
            )[:5]
            top_memory = sorted(
                processes, key=lambda p: p.info.get("memory_percent", 0), reverse=True
            )[:5]
    ...
    async def _monitor_continuous(
        self, interval: int, output_format: str, include_detailed: bool
    ) -> None:
        """Monitor system metrics continuously."""
        click.echo(
            f"Monitoring system metrics every {interval} seconds. Press Ctrl+C to stop."
        )
    ...
    def _output_text(self, metrics: Dict[str, Any]) -> None:
        ...
        click.echo(
            f"  Swap: {memory['swap_used_gb']}/{memory['swap_total_gb']} GB ({memory['swap_usage_percent']}%)"
        )
        ...
        click.echo(
            f"    Memory: {current['memory_percent']:.2f}% ({current['memory_info']['rss_mb']} MB)"
        )
...
class ProfilingCommand(AsyncCommand):
    ...
    async def execute_async(self, **kwargs: Any) -> None:
        ...
        # Start profiling
        click.echo(
            f"Starting {duration}s profiling session for {operation} operations..."
        )
    ...
    async def _collect_profiling_data(
    ...
    def _calculate_deltas(
        ...
        deltas = {
            "cpu_percent_change": end_metrics["cpu_percent"]
            - start_metrics["cpu_percent"],
            "memory_percent_change": end_metrics["memory_percent"]
            - start_metrics["memory_percent"],
            "process_count_change": end_metrics["process_count"]
            - start_metrics["process_count"],
        }

        # Calculate I/O deltas
        if start_metrics["disk_io"] and end_metrics["disk_io"]:
            deltas["disk_read_bytes"] = (
                end_metrics["disk_io"].read_bytes - start_metrics["disk_io"].read_bytes
            )
            deltas["disk_write_bytes"] = (
                end_metrics["disk_io"].write_bytes
                - start_metrics["disk_io"].write_bytes
            )

        if start_metrics["network_io"] and end_metrics["network_io"]:
            deltas["network_sent_bytes"] = (
                end_metrics["network_io"].bytes_sent
                - start_metrics["network_io"].bytes_sent
            )
            deltas["network_recv_bytes"] = (
                end_metrics["network_io"].bytes_recv
                - start_metrics["network_io"].bytes_recv
            )
...
def register(registry) -> None:
    ...
    performance_group = registry.register_group(
    ...
    registry.register_command(
        ...
        options=[
            ...
            click.Option(
                ["--include-detailed"], is_flag=True, help="Include detailed metrics"
            ),
            click.Option(
                ["--refresh-interval"],
                type=int,
                default=0,
                help="Continuous monitoring interval in seconds (0 for single snapshot)",
            ),
        ],
    )
    ...
    registry.register_command(
        ...
        options=[
            ...
            click.Option(
                ["--operation"],
                type=click.Choice(
                    ["general", "cpu_intensive", "memory_intensive", "io_intensive"]
                ),
                default="general",
                help="Type of operation to profile",
            ),
        ],
    )
