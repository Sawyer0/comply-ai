...
class ServiceDiscoveryCommand(AsyncCommand):
    ...
    def _get_configured_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        if hasattr(self.config_manager, "detector_orchestration"):
            services["detector-orchestration"] = {
                ...
                "host": getattr(
                    self.config_manager.detector_orchestration, "host", "localhost"
                ),
                "port": getattr(
                    self.config_manager.detector_orchestration, "port", 8001
                ),
                "endpoints": {
                    "health": "/health",
        ...
        if hasattr(self.config_manager, "analysis_module"):
            services["analysis-module"] = {
                ...
                "host": getattr(
                    self.config_manager.analysis_module, "host", "localhost"
                ),
    ...
    def _output_text(
        self, services: Dict[str, Dict[str, Any]], include_details: bool
    ) -> None:
...
class HealthAggregationCommand(AsyncCommand):
    ...
    async def execute_async(self, **kwargs: Any) -> None:
        ...
        # Aggregate results
        aggregated_health = self._aggregate_health_results(
            health_results, include_metrics
        )
    ...
    async def _check_all_services_health(
        self, services: Dict[str, Dict[str, Any]], timeout: int
    ...
    async def _check_service_health(
        self, client: httpx.AsyncClient, service_id: str, service: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _aggregate_health_results(
        self, health_results: Dict[str, Dict[str, Any]], include_metrics: bool
    ) -> Dict[str, Any]:
    ...
    def _output_text(self, aggregated_health: Dict[str, Any]) -> None:
        ...
        for service_id, result in aggregated_health["services"].items():
            ...
            click.echo(
                f"  {status_icon} {service_id}: {result.get('status', 'unknown')}"
            )
            if "error" in result:
...
def register(registry) -> None:
    ...
    service_group = registry.register_group(
        "services", "Service discovery and health aggregation"
    )
