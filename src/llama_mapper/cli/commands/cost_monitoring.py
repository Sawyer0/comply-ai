...
from ..utils import format_output, display_success, display_error, display_info
...
class CostMonitoringCommand(AsyncCommand):
    ...
    async def _get_cost_system(self) -> CostMonitoringFactory:
        ...
        if self.cost_system is None:
            ...
            self.cost_system = CostMonitoringFactory(cost_config)
    ...
    async def execute_async(self, **kwargs: Any) -> None:
        ...
        if format_type == "json":
            ...
            for anomaly in anomalies[:5]:  # Show last 5
                click.echo(f"  • {anomaly.anomaly_type}: ${anomaly.cost_value:.2f} "
...
class CostForecastCommand(CostMonitoringCommand):
    ...
    async def execute_async(self, **kwargs: Any) -> None:
        ...
        if format_type == "json":
            ...
            click.echo(f"Forecast Period: {forecast.forecast_period_start.date()} to {forecast.forecast_period_end.date()}")
            click.echo(f"Predicted Cost: ${forecast.predicted_cost:.2f}")
            click.echo(f"Confidence Level: {forecast.confidence_level:.1%}")
            click.echo(f"Confidence Interval: ${forecast.confidence_interval_lower:.2f} - ${forecast.confidence_interval_upper:.2f}")
...
def register(registry) -> None:
    ...
    registry.register_command(
        ...
        options=[
            ...
            click.Option(["--priority-min"], type=int, default=1, help="Minimum priority level"),
            click.Option(["--tenant-id"], help="Tenant ID to filter by"),
            click.Option(["--format", "fmt"], type=click.Choice(["json", "text"]), default="json", help="Output format"),
        ]
    )
