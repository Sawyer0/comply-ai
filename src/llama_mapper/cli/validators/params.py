...
class ParameterValidator:
    ...
    def validate_non_negative_integer(value: Any, param_name: str) -> int:
        ...
        try:
            ...
            if int_value < 0:
                raise CLIError(
                    f"{param_name} must be a non-negative integer, got: {value}"
                )
            return int_value
        except (ValueError, TypeError):
            raise CLIError(f"{param_name} must be an integer, got: {value}")
    ...
    def validate_float_range(
        value: Any, param_name: str, min_val: float = 0.0, max_val: float = 1.0
    ) -> float:
    ...
    @staticmethod
    def validate_regex(
        value: Any, param_name: str, pattern: str, description: str = ""
    ) -> str:
        """Validate that a value matches a regex pattern."""
        if not re.match(pattern, str(value)):
            desc = f" ({description})" if description else ""
            raise CLIError(
                f"{param_name} does not match required pattern{desc}, got: {value}"
            )
        return str(value)
    ...
    def validate_environment(environment: str) -> str:
        ...
        return ParameterValidator.validate_choice(
            environment, "Environment", valid_environments
        )
    ...
    def validate_output_format(format_type: str) -> str:
        ...
        return ParameterValidator.validate_choice(
            format_type, "Output format", valid_formats
        )
    ...
    def validate_host(host: str) -> str:
        ...
        if not (
            re.match(ip_pattern, host)
            or re.match(hostname_pattern, host)
            or host == "localhost"
        ):
