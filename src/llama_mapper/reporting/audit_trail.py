...
from ..data.frameworks import FrameworkMapper, FrameworkMapping
from ..data.taxonomy import TaxonomyLoader, Taxonomy
from ..data.detectors import DetectorConfigLoader
from .models import (
    AuditRecord, LineageRecord, ComplianceControlMapping, CoverageMetrics,
    ComplianceReport, CoverageReport, ReportMetadata
)
...
class AuditTrailManager:
    ...
    def create_lineage_record(self,
                             ...
                             tenant_id: Optional[str] = None) -> LineageRecord:
        """
        ...
        logger.debug(f"Created lineage record: {detector_name}:{original_label} -> {canonical_label}")
    ...
    def record_incident(self,
                       ...
                       detector_type: str,
                       tenant_id: Optional[str] = None,
                       severity: str = "medium") -> IncidentData:
        """
    ...
    def resolve_incident(self, incident_id: str) -> bool:
        ...
        for incident in self._incidents:
            if incident.incident_id == incident_id:
                ...
                logger.info(f"Resolved incident: {incident_id} in {incident.resolution_time_hours:.2f} hours")
                return True
    ...
    def generate_compliance_report(self,
        ...
        filtered_audit_records = self._filter_audit_records(tenant_id, start_date, end_date)
        filtered_lineage_records = self._filter_lineage_records(tenant_id, start_date, end_date)
        filtered_incidents = self._filter_incidents(tenant_id, start_date, end_date)
        
        # Generate coverage metrics
        coverage_metrics = self._calculate_coverage_metrics(filtered_incidents, frameworks)
        
        # Generate control mappings
        control_mappings = self._generate_control_mappings(filtered_audit_records, frameworks)
        
        # Generate framework summary
        framework_summary = self._generate_framework_summary(filtered_audit_records, frameworks)
    ...
    def generate_coverage_report(self,
                                tenant_id: Optional[str] = None,
                                include_detector_stats: bool = True) -> CoverageReport:
        """
        ...
        coverage_percentage = (len(covered_labels) / len(all_labels)) * 100 if all_labels else 0
        
        # Generate detector statistics
        detector_statistics = {}
        if include_detector_stats:
            detector_statistics = self._generate_detector_statistics(filtered_records, detector_configs)
        
        # Generate category breakdown
        category_breakdown = self._generate_category_breakdown(taxonomy, covered_labels, uncovered_labels)
    ...
    def get_lineage_for_label(self, canonical_label: str, tenant_id: Optional[str] = None) -> List[LineageRecord]:
        """
    ...
    def get_compliance_controls_for_label(self, taxonomy_label: str) -> Dict[str, List[Dict[str, str]]]:
        """
    ...
    def validate_compliance_mappings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_taxonomy_labels': len(all_labels),
            'mapping_coverage_percentage': (len(mapped_labels) / len(all_labels)) * 100 if all_labels else 0
        }
    ...
    def _filter_lineage_records(self,
                               tenant_id: Optional[str] = None,
                               start_date: Optional[datetime] = None,
                               end_date: Optional[datetime] = None) -> List[LineageRecord]:
    ...
    def _filter_incidents(self,
                         tenant_id: Optional[str] = None,
                         start_date: Optional[datetime] = None,
                         end_date: Optional[datetime] = None) -> List[IncidentData]:
    ...
    def _calculate_coverage_metrics(self,
                                   ...
                                   frameworks: Optional[List[str]] = None) -> CoverageMetrics:
        ...
        coverage_percentage = (covered_incidents / total_incidents * 100) if total_incidents > 0 else 100
        
        # Calculate MTTR
        mttr_hours = None
        if resolved_incidents:
            total_resolution_time = sum(i.resolution_time_hours or 0 for i in resolved_incidents)
        ...
        if frameworks:
            for framework in frameworks:
                framework_incidents = self._get_framework_incidents(incidents, framework)
                ...
                framework_specific_metrics[framework] = {
                    ...
                    'coverage_percentage': (len(framework_resolved) / len(framework_incidents) * 100) 
                                         if framework_incidents else 100
                }
    ...
    def _generate_control_mappings(self,
                                  ...
                                  frameworks: Optional[List[str]] = None) -> List[ComplianceControlMapping]:
    ...
    def _generate_framework_summary(self,
                                   audit_records: List[AuditRecord],
                                   frameworks: Optional[List[str]] = None) -> Dict[str, Dict[str, Any]]:
        ...
        for framework_name, count in framework_counts.items():
            summary[framework_name] = {
                ...
                'unique_controls': len(set(
                    mapping.split(':', 1)[1] for record in audit_records
                    for mapping in record.framework_mappings
    ...
    def _generate_detector_statistics(self,
                                     audit_records: List[AuditRecord],
                                     detector_configs: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """Generate detector statistics."""
        stats = {}
        
        # Count records per detector
        detector_counts = defaultdict(int)
        detector_confidence_scores = defaultdict(list)
        detector_methods = defaultdict(lambda: defaultdict(int))
        
        for record in audit_records:
            detector_counts[record.detector_type] += 1
            detector_confidence_scores[record.detector_type].append(record.confidence_score)
    ...
    def _generate_category_breakdown(self,
                                    ...
                                    uncovered_labels: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for category in all_categories:
            category_labels = [label for label in taxonomy.get_all_label_names() 
                             if label.startswith(f"{category}.")]
            
            covered_in_category = [label for label in category_labels if label in covered_labels]
            uncovered_in_category = [label for label in category_labels if label in uncovered_labels]
            
            breakdown[category] = {
                'total_labels': len(category_labels),
                'covered_labels': len(covered_in_category),
                'uncovered_labels': len(uncovered_in_category),
                'coverage_percentage': (len(covered_in_category) / len(category_labels) * 100) 
                                     if category_labels else 100
            }
        
        return breakdown
    
    def _get_framework_incidents(self, incidents: List[IncidentData], framework: str) -> List[IncidentData]:
    ...
    def _create_report_metadata(self, report_type: str, tenant_id: Optional[str] = None) -> ReportMetadata:
