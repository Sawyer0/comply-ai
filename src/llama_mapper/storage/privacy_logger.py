...
class PrivacyLogger:
    ...
    async def log_audit_trail(
        ...
        entry = PrivacyLogEntry(
            ...
            metadata=self._sanitize_metadata(
                {**(metadata or {}), "action": action, "resource": resource}
            ),
        )
    ...
    async def get_compliance_report(
        ...
        for entry in entries:
            ...
            if entry.detector_type:
                detector_counts[entry.detector_type] = (
                    detector_counts.get(entry.detector_type, 0) + 1
                )

            # Count by taxonomy hit
            if entry.taxonomy_hit:
                taxonomy_counts[entry.taxonomy_hit] = (
                    taxonomy_counts.get(entry.taxonomy_hit, 0) + 1
                )

            # Count by error category
            if entry.error_category:
                error_counts[entry.error_category] = (
                    error_counts.get(entry.error_category, 0) + 1
                )
        ...
        return {
            ...
            "audit_entries": [
                entry.to_dict() for entry in entries[:100]
    ...
    async def cleanup_old_entries(self, retention_days: int = 90) -> int:
        ...
        self._log_entries = [
            entry for entry in self._log_entries if entry.timestamp >= cutoff_time
        ]
