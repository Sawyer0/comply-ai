...
class TenantIsolationManager:
    ...
    def validate_tenant_access(
        ...
        if context.access_level == TenantAccessLevel.ADMIN:
            self._log_access_pattern(requesting_tenant, target_tenant, operation, "admin")
            return True
        
        # Shared access with explicit allowlist
        if (context.access_level == TenantAccessLevel.SHARED and 
            context.allowed_tenants and 
            target_tenant in context.allowed_tenants):
            self._log_access_pattern(requesting_tenant, target_tenant, operation, "shared")
            return True
    ...
    def apply_tenant_filter(
        ...
        if (tenant_context.access_level == TenantAccessLevel.SHARED and 
            tenant_context.allowed_tenants):
        ...
        self.logger.debug(
            ...
            filtered_query=filtered_query[:100] + "..." if len(filtered_query) > 100 else filtered_query
        )
    ...
    def update_tenant_config(self, tenant_id: str, config_or_overrides: Union[TenantConfig, Dict[str, Any]]) -> None:
        """
        ...
        if isinstance(config_or_overrides, dict):
            ...
            if 'confidence_threshold' in config_or_overrides:
                existing_config.confidence_threshold = config_or_overrides['confidence_threshold']
            if 'encryption_enabled' in config_or_overrides:
                existing_config.encryption_enabled = config_or_overrides['encryption_enabled']
            if 'storage_retention_days' in config_or_overrides:
                existing_config.storage_retention_days = config_or_overrides['storage_retention_days']
            if 'detector_whitelist' in config_or_overrides:
                existing_config.detector_whitelist = config_or_overrides['detector_whitelist']
            if 'detector_blacklist' in config_or_overrides:
                existing_config.detector_blacklist = config_or_overrides['detector_blacklist']
            ...
            if 'custom_taxonomy_mappings' in config_or_overrides:
                existing_config.custom_taxonomy_mappings = config_or_overrides['custom_taxonomy_mappings']
            ...
            if config.tenant_id != tenant_id:
                raise TenantIsolationError("Config tenant_id must match provided tenant_id")
    ...
    def _log_access_pattern(
        ...
        if len(self._access_patterns[requesting_tenant]) > 1000:
            self._access_patterns[requesting_tenant] = self._access_patterns[requesting_tenant][-1000:]
    ...
    def get_tenant_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_access_events": sum(len(patterns) for patterns in self._access_patterns.values()),
            "tenant_access_levels": {
                level.value: sum(
                    1 for ctx in self._active_contexts.values() 
