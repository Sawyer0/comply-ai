...
from .tenant_isolation import TenantIsolationManager, TenantContext
from .privacy_logger import PrivacyLogger
...
class StorageManager:
    ...
    async def _init_encryption(self) -> None:
        ...
        try:
            ...
            # Initialize Fernet with base64 encoded key
            import base64
            fernet_key = base64.urlsafe_b64encode(encryption_key)
            self._fernet = Fernet(fernet_key)
            
            self.logger.info("Encryption initialized", kms_enabled=bool(self.settings.kms_key_id))
    ...
    async def _setup_worm_policy(self) -> None:
        ...
        try:
            ...
            self.logger.info("S3 WORM policy configured", retention_years=lock_config['Rule']['DefaultRetention']['Years'])
            
        except ClientError as e:
            if 'InvalidBucketState' in str(e):
                self.logger.warning("Cannot enable Object Lock on existing bucket without versioning")
            else:
                self.logger.error("Failed to configure WORM policy", error=str(e))
                raise
    
    async def store_record(self, record: StorageRecord, tenant_context: Optional[TenantContext] = None) -> str:
        """
        ...
        try:
            ...
            if tenant_context:
                if not self.tenant_manager.validate_tenant_access(
                    ...
                    raise Exception(f"Tenant {tenant_context.tenant_id} cannot write to tenant {record.tenant_id}")
            
            # Create tenant-scoped record ID
            scoped_id = self.tenant_manager.create_tenant_scoped_record_id(record.tenant_id, record.id)
            original_id = record.id
            record.id = scoped_id
            # Encrypt sensitive data if encryption is enabled
            if self._fernet:
                encrypted_source = self._fernet.encrypt(record.source_data.encode()).decode()
                encrypted_mapped = self._fernet.encrypt(record.mapped_data.encode()).decode()
                record.source_data = encrypted_source
                record.mapped_data = encrypted_mapped
            ...
            s3_key = await self._store_in_s3(record)
            record.s3_key = s3_key
            
            # Update database record with S3 key
            await self._update_s3_key(record.id, s3_key)
            
            # Privacy-first audit logging (metadata only)
            self.privacy_logger.log_mapping_success(
                event_id=original_id,  # Use original ID as correlation
                tenant_id=record.tenant_id,
                detector_type=record.metadata.get("detector", "unknown"),
                taxonomy_hit=self._extract_taxonomy_from_mapped_data(record.mapped_data),
                confidence_score=self._extract_confidence_from_mapped_data(record.mapped_data),
                model_version=record.model_version,
                request_duration_ms=0,  # Would be calculated in actual implementation
                metadata={
                    "s3_key": s3_key,
            ...
            await self.privacy_logger.log_mapping_success(
                tenant_id=record.tenant_id or "unknown",
                detector_type=record.metadata.get("detector", "unknown"),
                taxonomy_hit=record.metadata.get("taxonomy_hit", "unknown"),
                confidence_score=record.metadata.get("confidence", 0.0),
                model_version=record.model_version,
                metadata={
                    "s3_key": s3_key,
                    "encrypted": record.encrypted
            ...
            self.logger.info("Record stored successfully", record_id=record.id, s3_key=s3_key, tenant_id=record.tenant_id)
            return original_id  # Return original ID to caller
            
        except Exception as e:
            self.logger.error("Failed to store record", record_id=record.id, error=str(e))
            raise
    
    async def retrieve_record(self, record_id: str, tenant_context: TenantContext) -> Optional[StorageRecord]:
        """
        ...
        try:
            # Create tenant-scoped record ID
            scoped_id = self.tenant_manager.create_tenant_scoped_record_id(tenant_context.tenant_id, record_id)
            # Try hot database first
            record = await self._retrieve_from_database(scoped_id, tenant_context)
            ...
            if record and not self.tenant_manager.validate_tenant_access(
            ...
            if record and record.encrypted and self._fernet:
                record.source_data = self._fernet.decrypt(record.source_data.encode()).decode()
                record.mapped_data = self._fernet.decrypt(record.mapped_data.encode()).decode()
            
            # Restore original record ID for caller
            if record:
                _, original_id = self.tenant_manager.extract_tenant_from_record_id(record.id)
                record.id = original_id
            
            return record
            
        except Exception as e:
            self.logger.error("Failed to retrieve record", record_id=record_id, error=str(e))
            raise
    ...
    async def _store_postgresql(self, record: StorageRecord) -> None:
        ...
        async with self._db_pool.acquire() as conn:
            ...
            """, record.id, record.source_data, record.mapped_data, 
                record.model_version, record.timestamp, record.metadata, record.tenant_id, record.encrypted)
    ...
    async def _store_clickhouse(self, record: StorageRecord) -> None:
        ...
        await asyncio.get_event_loop().run_in_executor(
            ...
            """,
            [(record.id, record.source_data, record.mapped_data, 
              record.model_version, record.timestamp, str(record.metadata), record.tenant_id, record.encrypted)]
        )
    ...
    async def _store_in_s3(self, record: StorageRecord) -> str:
        ...
        await asyncio.get_event_loop().run_in_executor(
            ...
            self._s3_client.put_object,
            {
                ...
                'ObjectLockRetainUntilDate': datetime.now(timezone.utc) + timedelta(days=365 * (self.settings.s3_retention_years or 7))
            }
        )
        
        return s3_key
    
    async def _retrieve_from_database(self, record_id: str, tenant_context: TenantContext) -> Optional[StorageRecord]:
    ...
    async def _retrieve_postgresql(self, record_id: str, tenant_context: TenantContext) -> Optional[StorageRecord]:
        ...
        filtered_query = self.tenant_manager.apply_tenant_filter(base_query, tenant_context)
    ...
    async def _retrieve_clickhouse(self, record_id: str, tenant_context: TenantContext) -> Optional[StorageRecord]:
    ...
    async def _retrieve_from_s3(self, record_id: str, tenant_context: TenantContext) -> Optional[StorageRecord]:
        ...
        try:
            ...
            for obj in response.get('Contents', []):
                if record_id in obj['Key']:
                    ...
                    if self.tenant_manager.validate_tenant_access(
                        tenant_context.tenant_id, record.tenant_id, "read"
                    ):
                        return record
            
        except Exception as e:
            self.logger.error("Failed to retrieve from S3", record_id=record_id, error=str(e))
    ...
    async def _cleanup_postgresql(self, cutoff_date: datetime) -> int:
        ...
        async with self._db_pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM storage_records WHERE timestamp < $1",
                cutoff_date
            )
