...
class ModelServer(ABC):
    ...
    def __init__(self, model_path: str, generation_config: GenerationConfig = None):
        ...
        try:
            ...
            async with self.session.get(f"{self.tgi_endpoint}/health") as response:
                if response.status == 200:
                    ...
                    raise RuntimeError(f"TGI server health check failed: {response.status}")
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    raise RuntimeError(f"TGI generation failed: {response.status} - {error_text}")
