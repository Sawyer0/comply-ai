...
class FallbackMapper:
    ...
    def load_detector_mappings(self):
        ...
        try:
            ...
            for config_file in config_files:
                if config_file.name in ["taxonomy.yaml", "frameworks.yaml", "schema.json"]:
                ...
                try:
                    ...
                    logger.warning(f"Failed to load detector config {config_file}: {str(e)}")
    ...
    def map(self, detector: str, output: str, reason: str = "low_confidence"):
        """
        ...
        for rule_output, canonical_label in detector_maps.items():
            if rule_output.lower() == output_lower:
                self.metrics_collector.increment_counter(
                    "fallback_case_insensitive_match_total",
                    {"detector": detector}
                )
                return MappingResponse(
                    taxonomy=[canonical_label],
        ...
        # Try partial match
        for rule_output, canonical_label in detector_maps.items():
            if rule_output.lower() in output_lower or output_lower in rule_output.lower():
                self.metrics_collector.increment_counter(
                    "fallback_partial_match_total",
                    {"detector": detector}
                )
                return MappingResponse(
                    taxonomy=[canonical_label],
        ...
        # No match found
        logger.warning(f"No rule mapping found for detector {detector} output: {output}")
        self.metrics_collector.increment_counter(
            "fallback_no_match_total",
            {"detector": detector}
        )
        return self._create_unknown_response(detector, output, reason)
    
    def _create_unknown_response(self, detector: str, output: str, reason: str = "unknown"):
        """
    ...
    def add_detector_mapping(self, detector: str, mappings: Dict[str, str]):
        """
    ...
    def get_mapping_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "detectors": {detector: len(maps) for detector, maps in self.detector_mappings.items()}
        }
                ...
                for label_key, count in label_dict.items():
                    if counter_name == "fallback_usage_total":
                        ...
                        if "detector=" in label_key:
                            ...
                            if detector:
                                fallback_stats["by_detector"][detector] = fallback_stats["by_detector"].get(detector, 0) + count
                            if reason:
                                fallback_stats["by_reason"][reason] = fallback_stats["by_reason"].get(reason, 0) + count
                        ...
                        fallback_stats["match_types"]["case_insensitive_matches"] += count
    ...
    def log_fallback_improvement_suggestions(self) -> None:
        ...
        if stats["by_detector"]:
            ...
            sorted_detectors = sorted(stats["by_detector"].items(), key=lambda x: x[1], reverse=True)
        ...
        if total_matches > 0:
            ...
            logger.info(f"  Exact matches: {match_types['exact_matches']} ({match_types['exact_matches']/total_matches*100:.1f}%)")
            logger.info(f"  Case-insensitive: {match_types['case_insensitive_matches']} ({match_types['case_insensitive_matches']/total_matches*100:.1f}%)")
            logger.info(f"  Partial matches: {match_types['partial_matches']} ({match_types['partial_matches']/total_matches*100:.1f}%)")
            logger.info(f"  No matches: {match_types['no_matches']} ({match_types['no_matches']/total_matches*100:.1f}%)")
            logger.info(f"  No mapping available: {match_types['no_mapping']} ({match_types['no_mapping']/total_matches*100:.1f}%)")
        
        # Provide improvement suggestions
        if match_types['no_matches'] > match_types['exact_matches']:
            logger.warning("High rate of unmatched outputs - consider expanding detector mappings")
        
        if match_types['partial_matches'] > match_types['exact_matches']:
            logger.warning("High rate of partial matches - consider adding exact mappings for common outputs")
        
        if stats["by_reason"].get("low_confidence", 0) > stats["total_fallback_usage"] * 0.5:
            logger.warning("High rate of low-confidence fallbacks - consider retraining model or adjusting confidence threshold")