...
class ConfidenceCalibrator:
    ...
    def _calibrate_temperature(self, logits: np.ndarray, labels: np.ndarray) -> None:
        ...
        def temperature_loss(temp):
            ...
            return -np.mean(correct * np.log(max_probs + 1e-8) + 
    ...
    def _calibrate_platt(self, logits: np.ndarray, labels: np.ndarray) -> None:
        ...
        def platt_loss(params):
            ...
            return -np.mean(correct * np.log(calibrated_probs + 1e-8) + 
    ...
    def apply_calibration(self, logits: np.ndarray) -> np.ndarray:
        ...
        if not self.is_calibrated:
            logger.warning("Calibrator not trained, returning uncalibrated probabilities")
        ...
        if self.method == "temperature":
            ...
            return probs * scale_factor[:, np.newaxis]
        elif self.method == "isotonic":
            probs = self._softmax(logits)
            max_probs = np.max(probs, axis=1)
...
class ConfidenceEvaluator:
    ...
    def evaluate_confidence(
        ...
        logger.debug(f"Confidence evaluation: {confidence:.3f} for label {predicted_label}")
    ...
    def should_use_fallback(self, confidence: float) -> bool:
        ...
        if use_fallback:
            logger.info(f"Using fallback: confidence {confidence:.3f} < threshold {self.threshold}")
        return use_fallback
    ...
    def update_threshold(self, new_threshold: float) -> None:
        ...
        logger.info(f"Confidence threshold updated: {old_threshold:.3f} -> {new_threshold:.3f}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'calibrator_trained': self.calibrator.is_calibrated if self.calibrator else False,
