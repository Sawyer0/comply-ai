...
from .interfaces import AlertManager, CostCalculator, MetricsStorage, ResourceMonitor
...
class CostBreakdown(BaseModel):
    ...
    tenant_id: Optional[str] = Field(
        default=None, description="Tenant ID for multi-tenant costs"
    )
...
class CostMetricsCollector:
    ...
    async def _check_cost_alerts(self, cost_metrics: CostMetrics) -> None:
        ...
        if (
            cost_metrics.usage.api_calls * cost_metrics.cost_per_unit["api_call"]
            > thresholds["api_call_cost"]
        ):
            await self._create_alert(
                alert_type="api_cost_high",
                threshold=thresholds["api_call_cost"],
                current_cost=cost_metrics.usage.api_calls
                * cost_metrics.cost_per_unit["api_call"],
                severity="low",
    ...
    async def get_cost_breakdown(
        ...
        period_metrics = await self.metrics_storage.get_metrics(
        ...
        storage_cost = sum(
            m.usage.storage_gb
            * m.cost_per_unit["storage_per_gb_month"]
            / (30 * 24 * 3600)
            for m in period_metrics
        )
        ...
        api_cost = sum(
            m.usage.api_calls * m.cost_per_unit["api_call"] for m in period_metrics
        )
...
class InMemoryMetricsStorage(MetricsStorage):
    ...
    def get_recent_metrics(
        self, count: int = DEFAULT_RECENT_METRICS_COUNT
    ) -> List[CostMetrics]:
