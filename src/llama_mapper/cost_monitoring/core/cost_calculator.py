...
from .metrics_collector import ResourceUsage, CostMetrics, CostBreakdown
...
class StandardCostCalculator(CostCalculator):
    ...
    def calculate_cost(self, usage: ResourceUsage) -> CostMetrics:
        ...
        total_cost = cpu_cost + memory_cost + gpu_cost + storage_cost + network_cost + api_cost
    ...
    def _calculate_gpu_cost(self, gpu_count: int, gpu_memory_gb: float) -> float:
        ...
        gpu_instance_cost = gpu_count * self.cost_per_unit["gpu_per_hour"] / SECONDS_PER_HOUR
        gpu_memory_cost = gpu_memory_gb * self.cost_per_unit["memory_per_gb_hour"] / SECONDS_PER_HOUR
    ...
    def _calculate_storage_cost(self, storage_gb: float) -> float:
        ...
        return storage_gb * self.cost_per_unit["storage_per_gb_month"] / SECONDS_PER_MONTH
    ...
    def calculate_breakdown(
        ...
        for usage in usage_list:
            ...
            total_compute_cost += self._calculate_gpu_cost(usage.gpu_count, usage.gpu_memory_gb)
    ...
    def __init__(self, cost_per_unit: Dict[str, float] = None):
        ...
        self.tiers = {
            "cpu": [
                ...
                {"threshold": 4, "rate": self.cost_per_unit["cpu_per_hour"] * 0.8},  # 20% discount
                {"threshold": 8, "rate": self.cost_per_unit["cpu_per_hour"] * 0.6},  # 40% discount
            ],
            "memory": [
                {"threshold": 0, "rate": self.cost_per_unit["memory_per_gb_hour"]},
                {"threshold": 16, "rate": self.cost_per_unit["memory_per_gb_hour"] * 0.8},
                {"threshold": 32, "rate": self.cost_per_unit["memory_per_gb_hour"] * 0.6},
            ],
        }
    ...
    def _calculate_tiered_cost(self, resource_type: str, amount: float) -> float:
        ...
        for i, tier in enumerate(tiers):
            ...
            if i == len(tiers) - 1:  # Last tier
                ...
                tier_amount = min(remaining_amount, next_tier_threshold - tier["threshold"])
