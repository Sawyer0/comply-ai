...
from ..core.metrics_collector import CostBreakdown, CostMetricsCollector
...
class CostOptimizationRecommendation(BaseModel):
    ...
    category: str = Field(
        description="Category of optimization (compute, storage, network, etc.)"
    )
...
class CostAnomaly(BaseModel):
    ...
    anomaly_type: str = Field(
        description="Type of anomaly (spike, drop, unusual_pattern)"
    )
...
class CostAnalyticsConfig(BaseModel):
    ...
    analysis_interval_hours: int = Field(
        default=24, description="How often to run analysis"
    )
    anomaly_detection_enabled: bool = Field(
        default=True, description="Enable anomaly detection"
    )
    forecasting_enabled: bool = Field(
        default=True, description="Enable cost forecasting"
    )
    recommendation_engine_enabled: bool = Field(
        default=True, description="Enable optimization recommendations"
    )
    anomaly_threshold: float = Field(
        default=2.0, description="Standard deviations for anomaly detection"
    )
    forecast_horizon_days: int = Field(default=30, description="Days to forecast ahead")
    min_data_points: int = Field(
        default=7, description="Minimum data points for analysis"
    )
...
class CostAnalytics:
    ...
    async def _generate_recommendations(self) -> None:
        ...
        if (
            cost_breakdown.compute_cost > cost_breakdown.total_cost * 0.6
        ):  # > 60% of total cost
        ...
        if (
            cost_breakdown.storage_cost > cost_breakdown.total_cost * 0.3
        ):  # > 30% of total cost
            recommendations.append(
                CostOptimizationRecommendation(
        ...
        if (
            cost_breakdown.network_cost > cost_breakdown.total_cost * 0.2
        ):  # > 20% of total cost
            recommendations.append(
                CostOptimizationRecommendation(
        ...
        if (
            cost_breakdown.api_cost > cost_breakdown.total_cost * 0.1
        ):  # > 10% of total cost
            recommendations.append(
                CostOptimizationRecommendation(
