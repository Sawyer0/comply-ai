...
class CostGuardrail(BaseModel):
    ...
    metric_type: str = Field(
        description="Type of metric to monitor (daily_cost, hourly_cost, api_calls, etc.)"
    )
    ...
    cooldown_minutes: int = Field(
        default=60, description="Cooldown period before re-triggering"
    )
    enabled: bool = Field(default=True, description="Whether the guardrail is enabled")
    tenant_id: Optional[str] = Field(
        default=None, description="Tenant-specific guardrail"
    )
...
class GuardrailViolation(BaseModel):
    ...
    metric_value: float = Field(
        description="Actual metric value that triggered the violation"
    )
...
class CostGuardrailsConfig(BaseModel):
    ...
    escalation_delay_minutes: int = Field(
        default=30, description="Delay before escalating actions"
    )
    max_violations_per_hour: int = Field(
        default=10, description="Maximum violations per hour before emergency actions"
    )
...
class CostGuardrails:
    ...
    async def _get_current_metric_value(self, guardrail: CostGuardrail) -> float:
        ...
        if metric_type == "daily_cost":
            ...
            if hasattr(self.metrics_collector, 'metrics_storage') and self.metrics_collector.metrics_storage:
    ...
    async def _handle_violation(
        self, guardrail: CostGuardrail, current_value: float
    ) -> None:
        ...
        if self._should_trigger_emergency_actions():
            violation.actions_taken.extend(  # pylint: disable=no-member
                self.config.emergency_actions
            )
    ...
    def _should_trigger_emergency_actions(self) -> bool:
        ...
        recent_violations = [v for v in self._violations if v.triggered_at >= hour_ago]
    ...
    async def _execute_action(
        self, action: GuardrailAction, violation: GuardrailViolation
    ) -> None:
    ...
    async def _block_requests(self, violation: GuardrailViolation) -> None:
        ...
        self.logger.critical(
            "Blocking requests due to critical cost guardrail violation"
        )
    ...
    def resolve_violation(self, violation_id: str) -> bool:
        ...
        for violation in self._violations:
            if violation.violation_id == violation_id:
                ...
                self.logger.info(
                    "Resolved guardrail violation", violation_id=violation_id
                )
                return True
