...
class CostMonitoringSystemConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Enable the entire cost monitoring system")
    ...
    alertmanager_enabled: bool = Field(default=True, description="Enable Alertmanager integration")
    
    # Cost thresholds (system-wide)
    daily_budget_limit: float = Field(default=1000.0, description="Daily budget limit")
    monthly_budget_limit: float = Field(default=30000.0, description="Monthly budget limit")
    emergency_stop_threshold: float = Field(default=5000.0, description="Emergency stop threshold")
    ...
    alerts_retention_days: int = Field(default=30, description="Alerts retention period")
    reports_retention_days: int = Field(default=365, description="Reports retention period")
    ...
    def validate_config(self) -> list[str]:
        ...
        if self.daily_budget_limit * 30 > self.monthly_budget_limit:
            issues.append("Daily budget limit * 30 should not exceed monthly budget limit")
        ...
        for channel, config in self.notification_channels.items():
            if config.get("enabled", False):
                if channel == "email" and not config.get("recipients"):
                    ...
                    issues.append("Slack notifications enabled but no webhook URL configured")
