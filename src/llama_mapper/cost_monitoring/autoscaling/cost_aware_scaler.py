...
class ScalingPolicy(BaseModel):
    ...
    scale_up_cooldown_minutes: int = Field(default=5, description="Cooldown after scale up")
    scale_down_cooldown_minutes: int = Field(default=15, description="Cooldown after scale down")
    cost_weight: float = Field(default=0.5, description="Weight given to cost in scaling decisions")
    performance_weight: float = Field(default=0.5, description="Weight given to performance in scaling decisions")
...
class ScalingDecision(BaseModel):
    ...
    executed_at: Optional[datetime] = Field(default=None, description="When the decision was executed")
...
class CostAwareScalingConfig(BaseModel):
    ...
    evaluation_interval_seconds: int = Field(default=60, description="How often to evaluate scaling decisions")
    cost_threshold_percent: float = Field(default=80.0, description="Cost threshold as percentage of budget")
    performance_threshold_percent: float = Field(default=70.0, description="Performance threshold as percentage")
    max_cost_increase_percent: float = Field(default=50.0, description="Maximum cost increase per scaling action")
    min_performance_improvement_percent: float = Field(default=20.0, description="Minimum performance improvement required")
    prediction_horizon_hours: int = Field(default=1, description="Hours to look ahead for cost predictions")
...
class CostAwareScaler:
    ...
    async def _evaluate_policy(self, policy: ScalingPolicy) -> Optional[ScalingDecision]:
        """Evaluate a specific scaling policy and return a decision."""
        current_instances = self._current_instances.get(policy.policy_id, policy.min_instances)
        ...
        scale_up_decision = await self._evaluate_scale_up(policy, current_instances, current_cost, current_performance)
        scale_down_decision = await self._evaluate_scale_down(policy, current_instances, current_cost, current_performance)
        ...
        for decision in [scale_up_decision, scale_down_decision]:
            if decision and decision.action != ScalingAction.NO_ACTION:
                score = self._calculate_decision_score(decision, current_cost, current_performance)
                if score > best_score:
                    best_score = score
    ...
    async def _evaluate_scale_up(
        ...
        cost_increase_percent = (cost_increase / current_cost * 100) if current_cost > 0 else 0
        
        if cost_increase_percent > self.config.max_cost_increase_percent:
            return None
        
        # Check if performance improvement justifies the cost
        predicted_performance = await self._predict_performance(policy, target_instances)
    ...
    async def _predict_cost(self, policy: ScalingPolicy, instances: int) -> float:
        ...
        cost_per_instance = base_cost / max(self._current_instances.get(policy.policy_id, 1), 1)
        
        return cost_per_instance * instances
    
    async def _predict_performance(self, policy: ScalingPolicy, instances: int) -> float:
    ...
    async def _scale_up_resources(self, decision: ScalingDecision) -> None:
        ...
        self.logger.info(f"Scaling up {decision.resource_type} to {decision.target_instances} instances")
    
    async def _scale_down_resources(self, decision: ScalingDecision) -> None:
        """Scale down resources."""
        # Implementation would depend on your infrastructure
        self.logger.info(f"Scaling down {decision.resource_type} to {decision.target_instances} instances")
    
    async def _scale_out_resources(self, decision: ScalingDecision) -> None:
        """Scale out resources (add more nodes/instances)."""
        # Implementation would depend on your infrastructure
        self.logger.info(f"Scaling out {decision.resource_type} to {decision.target_instances} instances")
    
    async def _scale_in_resources(self, decision: ScalingDecision) -> None:
        """Scale in resources (remove nodes/instances)."""
        # Implementation would depend on your infrastructure
        self.logger.info(f"Scaling in {decision.resource_type} to {decision.target_instances} instances")
