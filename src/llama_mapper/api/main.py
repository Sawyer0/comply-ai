...
from ..serving import create_model_server, GenerationConfig, JSONValidator, FallbackMapper
...
async def create_service():
    ...
    model_path = config_manager.get("model_path", "meta-llama/Llama-3-8B-Instruct")
    backend = config_manager.get("serving_backend", "vllm")
    ...
    schema_path = config_manager.get("schema_path", "pillars-detectors/schema.json")
    json_validator = JSONValidator(schema_path=schema_path)
    
    # Create fallback mapper
    detector_configs_path = config_manager.get("detector_configs_path", "pillars-detectors")
