...
class Provenance(BaseModel):
    ...
    raw_ref: Optional[str] = Field(None, description="Pointer/ID to the raw event in Splunk/Datadog/S3")
...
class DetectorRequest(BaseModel):
    ...
    detector: str = Field(..., description="Name of the detector that produced the output")
    output: str = Field(..., description="Raw output from the detector")
    metadata: Optional[Dict] = Field(None, description="Optional metadata about the detection")
...
class MappingResponse(BaseModel):
    ...
    confidence: float = Field(..., ge=0.0, le=1.0, description="Model-calibrated confidence in the mapping")
    ...
    @classmethod
    def validate_taxonomy_format(cls, v):
...
class BatchMappingResponse(BaseModel):
    ...
    errors: Optional[List[Dict]] = Field(None, description="Any errors encountered during batch processing")
