...
class TaxonomyCategory:
    ...
    def find_label_by_name(self, name: str) -> Optional[TaxonomyLabel]:
        ...
        # Check subcategories
        for subcategory in self.subcategories.values():
            found_label: Optional[TaxonomyLabel] = subcategory.find_label_by_name(name)
            if found_label:
                return found_label
...
class Taxonomy:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Taxonomy(version={self.version}, namespace={self.namespace}, "
...
class TaxonomyLoader:
    ...
    def __init__(self, taxonomy_path: Optional[Union[str, Path]] = None):
        ...
        self.taxonomy_path = (
            Path(taxonomy_path)
            if taxonomy_path
            else Path("pillars-detectors/taxonomy.yaml")
        )
    ...
    def load_taxonomy(self, force_reload: bool = False) -> Taxonomy:
        ...
        if (
            ...
            logger.debug(
                f"Taxonomy already loaded and up-to-date: {self.taxonomy_path}"
            )
    ...
    def _parse_categories(
        self, taxonomy: Taxonomy, categories_data: Dict[str, dict]
    ) -> None:
    ...
    def _parse_category(self, name: str, data: dict) -> TaxonomyCategory:
        ...
        category = TaxonomyCategory(name=name, description=data.get("description", ""))
