...
class DetectorConfig(BaseModel):
    ...
    notes: Optional[str] = Field(
        None, description="Additional notes about the detector"
    )
    maps: Dict[str, str] = Field(
        ..., description="Mapping from detector labels to taxonomy labels"
    )
    ...
    def validate_detector_name(cls, v: str) -> str:
        ...
        if not v or not v.replace("-", "").replace("_", "").isalnum():
            raise ValueError(
                "Detector name must contain only alphanumeric characters, hyphens, and underscores"
            )
        return v
...
class DetectorMapping:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"DetectorMapping(detector={self.detector}, version={self.version}, "
...
class DetectorConfigLoader:
    ...
    def __init__(
        ...
        """
        self.detectors_path = (
            Path(detectors_path) if detectors_path else Path("pillars-detectors")
        )
    ...
    def load_all_detector_configs(
        self, force_reload: bool = False
    ) -> Dict[str, DetectorMapping]:
        """
        ...
        if not self.detectors_path.exists():
            raise FileNotFoundError(
                f"Detectors directory not found: {self.detectors_path}"
            )
        ...
        current_scan_time = max(
            (
                f.stat().st_mtime
                for f in self.detectors_path.glob("*.yaml")
                if f.is_file()
            ),
            default=0,
        )
        ...
        if (
            not force_reload
            and self._detector_mappings
            and self._last_scan_time == current_scan_time
        ):
    ...
    def validate_all_mappings(
        ...
        for detector_name, mapping in self._detector_mappings.items():
            validation_results[detector_name] = mapping.validate_against_taxonomy(
                taxonomy
            )
    ...
    def get_mapping_coverage_report(
        self, taxonomy: Optional[Taxonomy] = None
    ) -> Dict[str, Any]:
        """
        ...
        category_coverage: Dict[str, Dict[str, Union[int, float]]] = {}
        ...
        for category in category_coverage.keys():
            ...
            covered_f = float(covered_val)
            category_coverage[category]["percentage"] = (
                (covered_f / total_f * 100.0) if total_f > 0.0 else 0.0
            )
        ...
        for detector_name, mapping in self._detector_mappings.items():
            for detector_label, canonical_label in mapping.maps.items():
                ...
                label_mappings[detector_label].append(
                    {"detector": detector_name, "canonical_label": canonical_label}
                )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"DetectorConfigLoader(path={self.detectors_path}, "
