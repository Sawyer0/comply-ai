...
class FrameworkConfig(BaseModel):
    ...
    frameworks: Dict[str, Dict[str, str]] = Field(..., description="Framework definitions")
    mappings: Dict[str, List[str]] = Field(..., description="Taxonomy to framework control mappings")
    ...
    @classmethod
    def validate_frameworks_not_empty(cls, v: Dict[str, Dict[str, str]]) -> Dict[str, Dict[str, str]]:
    ...
    def validate_mappings_format(cls, v: Dict[str, List[str]]) -> Dict[str, List[str]]:
        ...
        for taxonomy_label, controls in v.items():
            ...
            for control in controls:
                if ':' not in control:
                    raise ValueError(f"Control reference '{control}' must be in format 'Framework:ControlId'")
        return v
...
class FrameworkMapping:
    ...
    mappings: Dict[str, List[str]] = field(default_factory=dict)  # taxonomy_label -> [framework:control_id]
    file_path: Optional[Path] = None
    last_modified: Optional[float] = None
    
    def get_framework_controls_for_label(self, taxonomy_label: str) -> Dict[str, List[str]]:
        """
        ...
        """
        controls = self.mappings.get(taxonomy_label, [])
        framework_controls = {}
    ...
    def get_labels_for_framework_control(self, framework: str, control_id: str) -> List[str]:
        """
    ...
    def remove_mapping(self, taxonomy_label: str, framework: str, control_id: str) -> bool:
    ...
    def _remove_framework_from_mappings(self, framework_name: str) -> None:
        ...
        for taxonomy_label, controls in self.mappings.items():
            updated_controls = [c for c in controls if not c.startswith(f"{framework_name}:")]
    ...
    def __repr__(self) -> str:
        ...
        return (f"FrameworkMapping(version={self.version}, frameworks={len(self.frameworks)}, "
...
class FrameworkMapper:
    ...
    def __init__(self, 
        ...
        """
        self.frameworks_path = Path(frameworks_path) if frameworks_path else Path("pillars-detectors/frameworks.yaml")
    ...
    def load_framework_mapping(self, force_reload: bool = False) -> FrameworkMapping:
        ...
        if not force_reload and self._framework_mapping and self._last_modified == current_modified:
            logger.debug(f"Framework mapping already loaded and up-to-date: {self.frameworks_path}")
    ...
    def get_compliance_controls_for_label(self, taxonomy_label: str) -> Dict[str, List[Dict[str, str]]]:
        ...
        framework_controls = self._framework_mapping.get_framework_controls_for_label(taxonomy_label)
        result = {}
    ...
    def generate_compliance_report(self, 
        ...
        all_controls = {}  # framework -> set of controls
        
        for label in taxonomy_labels:
            controls = self.get_compliance_controls_for_label(label)
            
            if include_descriptions:
                report['framework_mappings'][label] = controls
            else:
                ...
                report['framework_mappings'][label] = simplified
    ...
    def add_framework_expansion(self, 
                               framework_name: str,
        ...
        if approval_required:
            logger.info(f"Framework expansion for '{framework_name}' requires approval workflow")
        ...
        if self._framework_mapping.has_framework(framework_name):
            ...
            logger.info(f"Expanded existing framework '{framework_name}' with {len(controls)} controls")
        else:
            new_framework = ComplianceFramework(framework_name, controls)
            self._framework_mapping.add_framework(new_framework)
            logger.info(f"Added new framework '{framework_name}' with {len(controls)} controls")
        
        return True
    
    def validate_against_taxonomy(self, taxonomy: Optional[Taxonomy] = None) -> Dict[str, List[str]]:
        """
    ...
    def _validate_framework_mapping(self, mapping: FrameworkMapping) -> None:
        ...
        if validation_results['invalid']:
            logger.warning(f"Invalid framework references found: {validation_results['invalid']}")
        ...
        if unused_frameworks:
            logger.warning(f"Defined frameworks not used in mappings: {unused_frameworks}")
