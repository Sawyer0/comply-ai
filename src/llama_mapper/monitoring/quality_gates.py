...
class QualityGateValidator:
    ...
    def __init__(
        ...
        self.golden_test_cases_path = golden_test_cases_path or Path("tests/golden_test_cases.json")
    ...
    def _load_thresholds(self) -> Dict[str, float]:
        ...
        if not self.config_path.exists():
            logger.warning(f"Quality gates config not found: {self.config_path}, using defaults")
        ...
        try:
            ...
            logger.info(f"Loaded quality gate thresholds for environment '{self.environment}'")
    ...
    def load_golden_test_cases(self) -> List[GoldenTestCase]:
        ...
        if not self.golden_test_cases_path.exists():
            logger.warning(f"Golden test cases file not found: {self.golden_test_cases_path}")
            return []
        ...
        try:
            ...
            for case_data in data.get("test_cases", []):
                test_cases.append(GoldenTestCase(
                    ...
                    expected_confidence_min=case_data.get("expected_confidence_min", 0.6),
    ...
    def validate_golden_test_coverage(self, test_cases: List[GoldenTestCase]) -> List[QualityGateResult]:
        """
        ...
        try:
            ...
            coverage_pct = (len(covered_categories) / len(categories)) * 100 if categories else 0
    ...
    async def run_golden_test_cases(self, test_cases: List[GoldenTestCase]) -> Tuple[List[QualityGateResult], Dict[str, Any]]:
        """
        ...
        for i, case in enumerate(test_cases):
            try:
                ...
                model_output = await self.model_server.generate_mapping(
                    ...
                    output=case.input_output,
                    metadata={}
                )
                
                processing_time = time.time() - start_time
                detailed_metrics["latency_measurements"].append(processing_time * 1000)  # Convert to ms
                ...
                if is_valid:
                    ...
                    detailed_metrics["confidence_scores"].append(parsed_output.confidence)
                    ...
                    if expected_labels:
                        ...
                        precision = len(intersection) / len(predicted_labels) if predicted_labels else 0
                        recall = len(intersection) / len(expected_labels)
                        f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0
                        
                        # Track F1 by detector
                        if case.detector not in detailed_metrics["f1_scores_by_detector"]:
                            detailed_metrics["f1_scores_by_detector"][case.detector] = []
                        detailed_metrics["f1_scores_by_detector"][case.detector].append(f1)
                        
                        # Check confidence threshold
                        confidence_ok = parsed_output.confidence >= case.expected_confidence_min
                        
                        if f1 >= 0.9 and confidence_ok:  # Both F1 and confidence thresholds met
                            detailed_metrics["passed_cases"] += 1
                        else:
                            detailed_metrics["failed_cases"] += 1
                            logger.warning(f"Test case {i} failed: F1={f1:.3f}, confidence={parsed_output.confidence:.3f}")
                    else:
                        detailed_metrics["failed_cases"] += 1
                        logger.warning(f"Test case {i} has no expected labels")
                else:
                    detailed_metrics["failed_cases"] += 1
                    logger.warning(f"Test case {i} failed schema validation: {validation_errors}")
        ...
        if detailed_metrics["total_cases"] > 0:
            schema_valid_pct = (detailed_metrics["schema_valid_cases"] / detailed_metrics["total_cases"]) * 100
            pass_rate = (detailed_metrics["passed_cases"] / detailed_metrics["total_cases"]) * 100
            
            # Schema validation gate
            results.append(QualityGateResult(
                passed=schema_valid_pct >= self.thresholds["schema_valid_percentage"],
    ...
    async def validate_all_quality_gates(self) -> Tuple[bool, List[QualityGateResult], Dict[str, Any]]:
        """
        ...
        logger.info(f"Total checks: {len(all_results)}, Failed: {len([r for r in all_results if not r.passed])}")
        
        return overall_passed, all_results, all_metrics
    
    def generate_quality_report(self, results: List[QualityGateResult], metrics: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        report = {
            ...
            "summary": {
                ...
                "critical_failures": len([r for r in failed_results if r.severity == "error"])
            },
            "results": [
                {
    ...
    def _generate_recommendations(self, failed_results: List[QualityGateResult]) -> List[str]:
        """Generate recommendations based on failed quality gates."""
        recommendations = []
        
        for result in failed_results:
            if "schema_valid" in result.metric_name:
                recommendations.append("Improve model training data quality and JSON schema validation")
            elif "f1_score" in result.metric_name:
                recommendations.append(f"Review training data for {result.metric_name.split('_')[-1]} detector")
            elif "latency" in result.metric_name:
                recommendations.append("Consider model quantization or hardware optimization")
            elif "golden_cases" in result.metric_name:
                recommendations.append(f"Add more golden test cases for {result.metric_name.split('_')[-1]} detector")
            elif "taxonomy_coverage" in result.metric_name:
                recommendations.append("Expand golden test cases to cover more taxonomy categories")
