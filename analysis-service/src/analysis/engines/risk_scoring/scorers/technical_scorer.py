...
Follows SRP by focusing solely on technical risk calculation using
CVSS-like methodology with enhanced factors.
"""
...
class TechnicalRiskScorer:
    ...
    async def calculate_risk(self, context: RiskCalculationContext) -> float:
        ...
        for finding in context.findings:
            try:
                ...
                logger.warning("Error calculating technical risk for finding %s: %s", 
    ...
    async def _calculate_finding_technical_risk(self, finding: Any, context: RiskCalculationContext) -> Optional[float]:
        ...
        try:
            ...
            confidence = max(0.1, getattr(finding, 'confidence', 0.5))  # Minimum confidence floor
            ...
            logger.error("Error in technical risk calculation for finding %s: %s", 
                        getattr(finding, 'finding_id', 'unknown'), e)
            return None

    async def _calculate_exploitability_factors(self, finding: Any, metadata: Dict[str, Any]) -> float:
