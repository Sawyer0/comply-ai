"""
Shared cost monitoring for analysis service.
...
class AnalysisCostMonitor:
    """Cost monitor for analysis service operations."""
    ...
    def __init__(self):
        ...
        logger.info("Analysis cost monitor initialized")
    ...
    def record_analysis_cost(
        ...
        """Record cost for analysis operations."""
        try:
            # Calculate cost based on processing time and model
            base_cost = Decimal(str(processing_time_ms / 1000.0 * 0.001))  # $0.001 per second
            
            # Add model-specific costs
            if model_used:
                if "gpt-4" in model_used.lower():
                    base_cost *= Decimal('2.0')  # 2x cost for GPT-4
                elif "claude" in model_used.lower():
                    base_cost *= Decimal('1.5')  # 1.5x cost for Claude

            # Create resource usage
            resource_usage = ResourceUsage(
                cpu_seconds=processing_time_ms / 1000.0,
                memory_mb=512,  # Estimated memory usage
                gpu_seconds=0.0,
                api_calls=1,
                tokens_processed=1000,  # Estimated token count
            )
            ...
            logger.error("Failed to record analysis cost", error=str(e))
...
_cost_monitor: Optional[AnalysisCostMonitor] = None


def get_shared_cost_monitor() -> AnalysisCostMonitor:
    """Get the global cost monitor instance."""
    global _cost_monitor
    if _cost_monitor is None:
        _cost_monitor = AnalysisCostMonitor()
