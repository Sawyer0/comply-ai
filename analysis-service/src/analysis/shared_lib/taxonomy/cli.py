...
def migration_commands(args):
    ...
    if args.migration_action == "plan":
        ...
        try:
            ...
            # Initialize database manager and migration executor
            db_manager = DatabaseManager()
            executor = MigrationExecutor(db_manager)
            
            # Execute migrations
            if args.schema_name:
                # Execute specific schema migrations
                results = executor.execute_schema_migrations(args.schema_name)
                print(f"Executed {len(results)} migrations for schema {args.schema_name}")
                for result in results:
                    status = "✓" if result.get("success") else "✗"
                    print(f"  {status} {result.get('migration_name', 'Unknown')}")
            else:
                # Execute all pending migrations
                results = executor.execute_all_pending_migrations()
                print(f"Executed {len(results)} total migrations")
                for schema, migrations in results.items():
                    print(f"\nSchema: {schema}")
                    for migration in migrations:
                        status = "✓" if migration.get("success") else "✗"
                        print(f"  {status} {migration.get('migration_name', 'Unknown')}")
                        
        except ImportError as e:
            print(f"Migration executor not available: {e}")
            print("Please ensure database migration dependencies are installed")
        except Exception as e:
