...
class AnalysisCircuitBreaker(ICircuitBreaker):
    """
    Circuit breaker implementation for analysis operations.

    Provides automatic failure detection and recovery for analysis service calls,
    ...
    def __init__(
        ...
        logger.info(
            "Analysis circuit breaker '%s' initialized with config: %s",
            name,
            self._config,
        )
    ...
    async def call(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        if self._state == CircuitState.OPEN:
            if self._should_attempt_reset():
                ...
                raise CircuitBreakerOpenException(
                    f"Analysis circuit breaker '{self._name}' is OPEN. "
        ...
        try:
            ...
            await self._handle_failure(TimeoutError("Analysis call timed out"))
            raise e
    ...
    async def _handle_failure(self, exception: Exception):
        ...
        logger.warning(
            "Analysis circuit breaker '%s' failure: %s. State: %s, Failures: %d",
            self._name,
            exception,
            self._state.value,
            self._failure_count,
    ...
    def _transition_to_open(self):
        ...
        if self._state != CircuitState.OPEN:
            ...
            logger.warning(
                "Analysis circuit breaker '%s' opened due to failures", self._name
            )
    ...
    def _transition_to_half_open(self):
        ...
        if self._state != CircuitState.HALF_OPEN:
            ...
            logger.info(
                "Analysis circuit breaker '%s' transitioned to half-open", self._name
            )
    ...
    def _transition_to_closed(self):
        ...
        if self._state != CircuitState.CLOSED:
            ...
            logger.info(
                "Analysis circuit breaker '%s' closed - service recovered", self._name
            )
    ...
    def reset(self):
        ...
        self._transition_to_closed()
        logger.info("Analysis circuit breaker '%s' manually reset", self._name)

    def force_open(self):
        """Manually force circuit breaker to open state."""
        self._transition_to_open()
        logger.warning("Analysis circuit breaker '%s' manually opened", self._name)
