...
class S3BackupMonitor:
    ...
    def check_backup_health(self, database_type: str, expected_frequency_hours: int = 24) -> Dict:
        ...
        try:
            ...
            backup_age = now - latest_backup['LastModified'].replace(tzinfo=timezone.utc)
...
class AlertManager:
    ...
    def send_alert(self, subject: str, message: str, recipients: Optional[List[str]] = None) -> bool:
...
class BackupMonitor:
    ...
    def check_all_backups(self) -> Dict:
        ...
        for db_type in databases:
            expected_frequency = self.config.get('databases', {}).get(db_type, {}).get('expected_frequency_hours', 24)
    ...
    def send_alerts_if_needed(self, health_check: Dict) -> bool:
        ...
        for db_type, health in health_check['databases'].items():
            if health['status'] in ['warning', 'error']:
                subject = f"Backup Alert: {db_type.upper()} - {health['status'].upper()}"
                ...
                if self.alert_manager.send_alert(subject, message, recipients):
                    alerts_sent = True
                    logger.info(f"Alert sent for {db_type} backup issue")
...
def main():
    ...
    parser.add_argument('--alerts', action='store_true', help='Send alerts if issues found')
    ...
    if args.check:
        ...
        health_check = monitor.check_all_backups()
        logger.info(f"Backup health check completed: {health_check['overall_status']}")
        ...
        report = monitor.generate_report()
        report_file = f"/var/log/backup-report-{datetime.now().strftime('%Y%m%d-%H%M%S')}.txt"
