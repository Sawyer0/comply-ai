...
def run_http_replay_tests(
        ...
        for test in failed_tests:
            ...
            for validation in test.get("validations", []):
                if not validation["passed"]:
                    print(
                        f"   - {validation['field']}: expected {validation['expected']}, got {validation['actual']}"
                    )
        print()
    ...
    if summary["pass_rate"] < min_pass_rate:
        print(
            f"❌ Pass rate {summary['pass_rate']:.2%} is below minimum {min_pass_rate:.2%}"
        )
        if fail_on_error:
            sys.exit(1)
    else:
        print(
            f"✅ Pass rate {summary['pass_rate']:.2%} meets minimum requirement {min_pass_rate:.2%}"
        )

    # Check for any failures
    if summary["failed_tests"] > 0 or summary["error_tests"] > 0:
        print(
            f"❌ {summary['failed_tests']} failed tests and {summary['error_tests']} error tests"
        )
        if fail_on_error:
            sys.exit(1)
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run HTTP replay tests for Analysis Module API"
    )
    parser.add_argument(
        "--golden-cases",
