...
def run_http_replay_tests(
    ...
    cmd = [
        ...
        "--golden-cases", golden_cases_path,
...
def run_lint_checks() -> Dict[str, Any]:
    ...
    cmd = ["python", "-m", "flake8", "src/llama_mapper/analysis/", "--max-line-length=88"]
    result = run_command(cmd)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run comprehensive analysis module tests")
    parser.add_argument(
        "--no-lint",
        action="store_true",
    ...
    parser.add_argument(
        "--replay-only",
        action="store_true",
        help="Run only HTTP replay tests"
    )
