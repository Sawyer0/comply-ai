...
def test_created_keys():
    ...
    for key_data in keys:
        ...
        validated_key = manager.validate_api_key(
            key_data["api_key"], [APIKeyScope.ANALYZE]  # Test with analyze scope
        )
        ...
        if "admin" in key_data["scopes"]:
            admin_validated = manager.validate_api_key(
                key_data["api_key"], [APIKeyScope.ADMIN]
            )
            if admin_validated:
        ...
        for i in range(3):
            rate_limited_key = manager.validate_api_key(
                key_data["api_key"], [APIKeyScope.ANALYZE]
            )
            if rate_limited_key:
                print(
                    f"   Request {i+1}: ✅ (usage count: {rate_limited_key.usage_count})"
                )
            else:
                print(f"   Request {i+1}: ❌")
...
def test_key_rotation():
    ...
    if new_response:
        ...
        old_validated = manager.validate_api_key(
            test_key["api_key"], [APIKeyScope.ANALYZE]
        )
