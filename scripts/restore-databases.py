...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("/var/log/restore.log")],
)
    ...
    def download_backup_directory(self, s3_prefix: str, local_path: str) -> bool:
        ...
        try:
            ...
            if "Contents" not in response:
                logger.error("No objects found with prefix: %s", s3_prefix)
                return False
            ...
            for obj in response["Contents"]:
                ...
                logger.info("Downloaded %s to %s", s3_key, local_file)

            return True

        except (ClientError, NoCredentialsError) as e:
            logger.error("Failed to download directory %s from S3: %s", s3_prefix, e)
            return False
...
class PostgreSQLRestore:
    ...
    def restore_backup(self, backup_path: str, drop_existing: bool = False) -> bool:
        ...
        try:
            ...
            # Restore backup
            logger.info("Restoring PostgreSQL backup from %s", backup_path)
            restore_cmd = [
                "pg_restore",
                "-h",
                self.host,
            ...
            result = subprocess.run(
                restore_cmd, stderr=subprocess.PIPE, env=env, check=True
            )
            ...
            logger.error("PostgreSQL restore failed: %s", e.stderr.decode())
            return False
...
class ClickHouseRestore:
    ...
    def restore_backup(self, backup_path: str, drop_existing: bool = False) -> bool:
        ...
        try:
            ...
            logger.info("Restoring ClickHouse backup from %s", backup_path)
            logger.info("Backup contains %s tables", len(metadata["tables"]))

            # Restore each table
            for table_info in metadata["tables"]:
                table_name = table_info["name"]
                logger.info("Restoring table: %s", table_name)
                ...
                if schema_file.exists():
                    ...
                    logger.info("Restored schema for table %s", table_name)
                ...
                if data_file.exists() and data_file.stat().st_size > 0:
                    ...
                    if len(lines) > 1:  # Has header and data
                        ...
                        logger.info(
                            "Restored %s rows to table %s", len(lines) - 1, table_name
                        )
                    else:
                        logger.info("Table %s has no data to restore", table_name)

            logger.info("ClickHouse restore completed successfully")
            return True

        except Exception as e:
            logger.error("ClickHouse restore error: %s", e)
            return False
...
class RedisRestore:
    ...
    def restore_backup(self, backup_path: str, flush_existing: bool = False) -> bool:
        ...
        try:
            ...
            logger.info(
                "Redis backup file copied. Redis will be restarted by the container orchestration."
            )

            return True

        except Exception as e:
            logger.error("Redis restore error: %s", e)
            return False
...
class DatabaseRestoreManager:
    ...
    def restore_backup(
        self, database_type: str, s3_key: str, drop_existing: bool = False
    ) -> bool:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if database_type == "postgresql":
                    ...
                    success = self.s3_manager.download_backup_directory(
                        s3_key, str(backup_dir)
                    )
                    if not success:
                        return False

                    return self.clickhouse.restore_backup(
                        str(backup_dir), drop_existing
                    )
            ...
            logger.error("Restore failed for %s: %s", database_type, e)
            return False
...
def main():
    ...
    parser.add_argument(
        "--database",
        choices=["postgresql", "clickhouse", "redis"],
        required=True,
        help="Database to restore",
    )
    parser.add_argument(
        "--backup", help="Specific backup to restore (S3 key or timestamp)"
    )
    parser.add_argument("--list", action="store_true", help="List available backups")
    parser.add_argument(
        "--drop-existing", action="store_true", help="Drop existing data before restore"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.list:
        ...
        if not backups:
            logger.info("No backups found for %s", args.database)
        else:
            logger.info("Available %s backups:", args.database)
            for backup in backups:
                logger.info(
                    "  %s - %s bytes - %s",
                    backup["timestamp"],
    ...
    if args.backup.startswith("backups/"):
        ...
        if args.database == "postgresql":
            ...
            logger.error("Unsupported database type: %s", args.database)
            sys.exit(1)

    if args.dry_run:
        logger.info("DRY RUN: Would restore %s from %s", args.database, s3_key)
        if args.drop_existing:
            logger.info("DRY RUN: Would drop existing data")
        return

    # Perform restore
    logger.info("Starting restore of %s from %s", args.database, s3_key)

    success = restore_manager.restore_backup(args.database, s3_key, args.drop_existing)

    if not success:
        logger.error("Restore failed for %s", args.database)
        sys.exit(1)

    logger.info("Successfully restored %s", args.database)
