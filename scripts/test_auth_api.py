...
def create_test_app():
    ...
    request = APIKeyRequest(
        tenant_id="test-tenant", name="Test API Key", scopes=["analyze", "admin"]
    )
    ...
    auth_dependency = APIKeyAuthDependency(
        api_key_manager=api_key_manager, required_scopes={APIKeyScope.ANALYZE}
    )
    ...
    async def admin_endpoint(
        auth: dict = Depends(
            APIKeyAuthDependency(
                api_key_manager=api_key_manager, required_scopes={APIKeyScope.ADMIN}
            )
        )
    ):
...
def test_auth_api():
    ...
    )  # Should work because our test key has admin scope
