    ...
    for detector, templates in detector_test_templates.items():
        ...
        for base_input, taxonomy, confidence, description in templates:
            ...
            variations = generate_test_variations(
                base_input, taxonomy, confidence, description
            )

            template_cases = 0
            for variation in variations:
                if (
                    detector_cases >= 100 or template_cases >= cases_per_template
                ):  # Limit per template and detector
                    break
...
def generate_test_variations(
    base_input: str, taxonomy: List[str], base_confidence: float, description: str
) -> List[Dict[str, Any]]:
    ...
    # Base case
    variations.append(
        {"input": base_input, "confidence": base_confidence, "description": "base case"}
    )
