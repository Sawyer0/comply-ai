...
def run_orchestration_tests(verbose: bool = False, coverage: bool = False) -> bool:
    ...
    comprehensive_test = Path(
        "../../tests/integration/test_detector_orchestration_comprehensive.py"
    )
...
def run_end_to_end_tests(verbose: bool = False) -> bool:
    ...
    cmd = [
        ...
        "-m",
        "pytest",
        "tests/integration/test_end_to_end_orchestration_mapper.py",
    ]
...
def generate_openapi_docs() -> bool:
    ...
    exit_code, stdout, stderr = run_command(
        [
            "python",
            "scripts/export_orchestration_openapi.py",
            "--output",
            "detector-orchestration/docs/openapi.yaml",
        ]
    )
...
def main():
    ...
    parser.add_argument(
        "--docs", "-d", action="store_true", help="Generate OpenAPI documentation"
    )
    parser.add_argument(
        "--skip-tests",
        action="store_true",
