...
class WeeklyEvaluationRunner:
    ...
    async def initialize(self):
        ...
        try:
            ...
            golden_dataset_path = os.getenv(
                "GOLDEN_DATASET_PATH", "/app/data/golden-dataset.json"
            )
            ...
            logger.error("Failed to initialize weekly evaluation runner: %s", e)
            raise
    ...
    async def run_scheduled_evaluations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for schedule in schedules:
                ...
                try:
                    ...
                    if next_run <= current_time:
                        logger.info("Running scheduled evaluation %s", schedule_id)

                        result = (
                            await self.weekly_eval_service.run_scheduled_evaluation(
                                schedule_id
                            )
                        )
                        results.append(result)

                        logger.info("Completed evaluation %s", schedule_id)
                    else:
                        logger.debug(
                            "Evaluation %s not due yet (next run: %s)",
                            schedule_id,
                            next_run,
                        )

                except Exception as e:
                    logger.error("Failed to run evaluation %s: %s", schedule_id, e)
                    results.append(
                        {
            ...
            logger.error("Failed to run scheduled evaluations: %s", e)
            raise
    ...
    async def run_specific_evaluation(self, schedule_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info("Running specific evaluation %s", schedule_id)

            result = await self.weekly_eval_service.run_scheduled_evaluation(
                schedule_id
            )
...
def main(schedule_id: Optional[str], dry_run: bool, verbose: bool):
    ...
    async def run():
        ...
        if dry_run:
            ...
            if due_evaluations:
                logger.info("Found %s due evaluations:", len(due_evaluations))
                for eval_info in due_evaluations:
                    logger.info(
                        "  - %s (tenant: %s)",
                        eval_info["schedule_id"],
                        eval_info["tenant_id"],
                    )
            else:
        ...
        if schedule_id:
            ...
            logger.info(
                "Evaluation result: %s", json.dumps(result, indent=2, default=str)
            )
        else:
            ...
            if results:
                logger.info("Completed %s evaluations:", len(results))
                for result in results:
                    status = result.get("status", "unknown")
                    schedule_id = result.get("schedule_id", "unknown")
                    logger.info("  - %s: %s", schedule_id, status)
            else:
    ...
    try:
        ...
        logger.error("Failed to run weekly evaluations: %s", e)
        sys.exit(1)
