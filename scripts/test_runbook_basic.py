...
async def test_runbook_executor_basic():
    ...
    with (
        patch(
            "src.llama_mapper.analysis.monitoring.metrics_collector.AnalysisMetricsCollector"
        ) as mock_metrics,
        patch(
        ...
        patch(
            "src.llama_mapper.analysis.infrastructure.opa_generator.OPAPolicyGenerator"
        ) as mock_opa,
        patch(
            "src.llama_mapper.analysis.quality.quality_alerting_system.QualityAlertingSystem"
        ) as mock_quality,
        patch("src.llama_mapper.cost_monitoring.CostMonitoringSystem") as mock_cost,
        patch(
            "src.llama_mapper.analysis.infrastructure.auth.APIKeyManager"
        ) as mock_auth,
        patch(
            "src.llama_mapper.analysis.security.waf.engine.rule_engine.WAFRuleEngine"
        ) as mock_waf,
        patch(
            "src.llama_mapper.analysis.domain.services.WeeklyEvaluationService"
        ) as mock_eval,
    ):
        ...
        spec = importlib.util.spec_from_file_location(
            "runbook_executor", project_root / "scripts" / "runbook-executor.py"
        )
