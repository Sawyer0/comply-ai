...
@click.option(
    "--db-backend",
    default="postgresql",
    help="Database backend (postgresql/clickhouse)",
)
@click.option("--skip-s3", is_flag=True, help="Skip S3 setup")
@click.option("--skip-db", is_flag=True, help="Skip database setup")
def setup_storage(
    bucket_name: str, region: str, db_backend: str, skip_s3: bool, skip_db: bool
):
...
async def setup_s3_bucket(settings: Settings):
    ...
    try:
        # Create S3 client
        s3_client = boto3.client("s3", region_name=settings.storage.aws_region)
                ...
                if settings.storage.aws_region == "us-east-1":
                    ...
                    s3_client.create_bucket(
                        ...
                        CreateBucketConfiguration={
                            "LocationConstraint": settings.storage.aws_region
                        },
                    )
