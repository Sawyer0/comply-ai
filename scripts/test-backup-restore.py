...
def test_backup_script():
    ...
    spec = importlib.util.spec_from_file_location("backup_databases", project_root / "scripts" / "backup-databases.py")
    backup_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(backup_module)
    
    # Mock all external dependencies
    with patch.object(backup_module, 'boto3') as mock_boto3, \
        ...
        config = {
            ...
            'postgresql': {'host': 'localhost', 'port': 5432, 'database': 'test', 'user': 'test', 'password': 'test'},
            'clickhouse': {'host': 'localhost', 'port': 9000, 'database': 'test', 'user': 'test', 'password': 'test'},
...
def test_restore_script():
    ...
    import importlib.util
    spec = importlib.util.spec_from_file_location("restore_databases", project_root / "scripts" / "restore-databases.py")
    restore_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(restore_module)
    
    # Mock all external dependencies
    with patch.object(restore_module, 'boto3') as mock_boto3, \
        ...
        mock_s3_client.list_objects_v2.return_value = {
            'Contents': [
                {'Key': 'backups/llama-mapper/postgresql/2024-01-01-12-00-00.dump', 'Size': 1024000, 'LastModified': '2024-01-01T12:00:00Z'}
            ]
        }
        mock_boto3.client.return_value = mock_s3_client
