...
class MapperRetryManager(IRetryManager):
    """
    Retry manager for mapper operations with exponential backoff.
    ...
    def __init__(
        ...
        logger.info("Mapper retry manager initialized with config: %s", self._config)
    ...
    def with_retry(**kwargs):
        ...
        def decorator(func):
            ...
            async def wrapper(*args, **func_kwargs):
                ...
                retry_manager = MapperRetryManager(config)
    ...
    async def execute_with_retry(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        for attempt in range(1, self._config.max_attempts + 1):
            ...
            try:
                ...
                logger.debug(
                    "Mapper function %s succeeded on attempt %s",
                    func.__name__,
                    attempt,
                )
                ...
                if not self._should_retry(e, attempt):
                    logger.warning(
                        "Mapper function %s failed with non-retryable exception: %s",
                        func.__name__,
                        e,
                    )
                    raise
                ...
                if attempt >= self._config.max_attempts:
                    logger.error(
                        "Mapper function %s failed after %d attempts: %s",
                        func.__name__,
                        attempt,
                        e,
                    )
                ...
                logger.warning(
                    "Mapper function %s failed on attempt %d: %s. Retrying in %.2fs...",
                    func.__name__,
                    attempt,
                    e,
                    delay,
